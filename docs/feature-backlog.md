...
- ✅ `/blog` pipeline — 4 real posts published, drafts pipeline emptied:
  - How we built non-custodial card issuance on Solana (10 min)
  - Anatomy of a Obolus order (8 min)
...
- `examples/README.md` had `npx -y obolus/mcp` which is not a
  valid package spec. The CLI default subcommand is `mcp`
  (`sdk/src/cli.ts:17`) so `npx -y obolus` is correct. Added a
...
agent→obolus Solana receiver→obolus treasury pays VCC.
...
- **Lightning Network rail.** Alongside Solana, support LN invoices for
...
- **Rust SDK.** Same, plus natively aligns with Solana contract tooling.
- **`obolus dev` local emulator.** A local server that mimics the API so
  agent developers can test without burning real USDC. Fake cards, fake
  Solana, fast iteration.
- **Time-travel debugging in the CLI.** `obolus orders inspect <id>` shows a
...
- **Penetration test** by an independent firm. Published summary on the
  Security page.
- **Status page.** `status.obolus.xyz` is referenced in the Terms already —
...
- **Proof-of-reserves / on-chain transparency.** Publish a public Solana
  account address holding the operational float, so anyone can verify
  Obolus is solvent in real time. Strongest possible signal for a
...
- **MCP marketplace / directory presence.** As the MCP ecosystem consolidates
  into directories, make sure Obolus is listed in every major one with
...
- **Developer-focused content series.** Weekly technical post:
  "How we built non-custodial card issuance on Solana",
  "Anatomy of a Obolus order",
...
- **Comparison page.** "Obolus vs Ramp vs Lithic for agent workloads."
...
- **Auto-generated SDK reference docs.** Loops 1–4 have caught four
  ...
  made-up or renamed SDK symbols (`npx obolus revoke`,
  `obolus-mcp`, `asset:` vs `paymentAsset`, `createWallet` vs
...
- **Author / team page.** The anatomy post is bylined "Obolus
...
- **Integration gallery.** `/built-with` showcasing agents built on
  Obolus with real screenshots and code snippets.
...
- **"Powered by Obolus" badge** that third-party integrators can
...
- **Proof-of-reserves dashboard.** Surface the on-chain treasury
  balance publicly so anyone can verify Obolus is solvent. Pairs
...
- **Programmable card lifecycle via Solana contract.** Let an operator
...
- **Card-backed credit line.** Agents with a proven history get a small
  ...
  the Solana payment first. High-trust, limited blast radius.
- **Zero-knowledge privacy for agent identity.** Issue a card where the
  agent's identity is provable to Obolus but not disclosed to the
...
- **Case study format.** One-page interview with an operator who
  shipped a real agent on Obolus — problem, solution, metrics. Lives
...
- **Glossary of agent payment terms.** `/docs/glossary` — SSE, OWS,
  PAN, CVV, token account, Solana, Solana memo, non-custodial, PSP,
