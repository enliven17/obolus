// `obolus purchase --amount 10 [--asset sol|usdc]` — one-command
// card purchase. Loads creds from the on-disk config written by
// `obolus onboard`, opens the order, pays the Solana contract
...
// SDK surface area.
//
// Resume: if a purchase fails mid-flight (Solana RPC timeout, network
// blip, etc.), the order id is saved to ~/.obolus/last-order and the
// user can retry with `obolus purchase --resume <order-id>`.
...
function usage(): void {
  process.stderr.write(`Usage: obolus purchase --amount <USDC> [--asset sol|usdc]
       obolus purchase --resume <order-id>

Buys a virtual Visa card for the given USD value using the credentials
and wallet set up by 'obolus onboard'. Reads ~/.obolus/config.json
for the api key, wallet name, vault path, and passphrase env var.

Options:
  -a, --amount <USDC>        Card value in USD. Required for new purchases.
  --asset sol|usdc           Which asset to pay with. Default: auto — picks
                             USDC if the wallet has enough USDC to cover the
                             order, otherwise pays in SOL. Pass an explicit
                             value to force one.
...
Examples:
  obolus purchase --amount 10                 # $10 card paid in SOL
  obolus purchase --amount 5 --asset usdc
  obolus purchase --resume a94d18cc-...       # pick up an interrupted purchase
`);
}

function lastOrderFile(): string {
  const dir = process.env.OBOLUS_CONFIG_DIR || path.join(os.homedir(), '.obolus');
 ...
 * to say anything".
 *
 * - `orderId`: the obolus order id (always present)
 * - `txHash`:  Solana tx hash if the client-side submit got far enough
 *              to capture one. Absent if the error fired before submit
 *              (e.g. token account add failed).
 ...
 *              the Solana inclusion window, a still-missing tx is
...
export async function purchaseCommand(argv: string[]): Promise<number> {
  ...
  if (args.assetInvalid) {
    process.stderr.write(
      `error: --asset must be 'sol', 'usdc', or 'auto' (got: ${args.assetInvalid})\n`,
    );
    return 2;
  }
  ...
  if (!config) {
    process.stderr.write(
      `error: no obolus config found at ~/.obolus/config.json

Run 'obolus onboard --claim <code>' first to set up credentials.
Your operator can mint a claim code from https://obolus.xyz/dashboard.
`,
    );
    return 1;
  }
  ...
  if (!walletName) {
    process.stderr.write(
      'error: no wallet_name in ~/.obolus/config.json and no --wallet-name passed.\n' +
        "Either pass --wallet-name <name>, or re-run 'obolus onboard --claim <code>'\n" +
  ...
  //     order, else pick SOL
  let paymentAsset: 'sol' | 'usdc';
  if (args.asset) {
    paymentAsset = args.asset;
  } else if (args.resume) {
    paymentAsset = 'sol'; // unused on resume, any value is fine
  } else {
    try {
      ...
      if (usdcBal >= wantUsdc && wantUsdc > 0) {
        ...
        if (usdcBal > 0) {
          process.stdout.write(
            `→ Auto-picked SOL (wallet has only ${usdcBal.toFixed(2)} USDC; needs $${wantUsdc.toFixed(2)})\n`,
          );
        } else {
          process.stdout.write(`→ Auto-picked SOL (no USDC in wallet)\n`);
        }
      }
    } catch {
      // Solana RPC down or unactivated wallet — fall back to SOL (no
      // token account required) and let payViaContractOWS surface the real
      // error if the wallet really has nothing.
      paymentAsset = 'sol';
      process.stdout.write(
        `→ Could not read balance from Solana RPC — defaulting to SOL. Pass --asset usdc to override.\n`,
      );
    }
  }
  ...
  try {
    ...
    if (err instanceof ResumableError) {
      ...
      process.stderr.write(`\nerror: ${err.message}\n`);
      process.stderr.write(
        `\nYour payment may still be processing on-chain. The obolus backend will\n` +
          `credit the order if the transaction finalizes. Resume with:\n\n` +
          `  obolus purchase --resume ${err.orderId}\n\n` +
          `(saved to ~/.obolus/last-order)\n`,
      );
      return 1;
    }
