// `obolus onboard --claim <code>` — one-shot agent setup.
//
...
// 2. Persist the api key + api_url in ~/.obolus/config.json (0600)
...
// 5. Print the Solana address + next steps.

import { assertSafeBaseUrl, loadObolusConfig, saveObolusConfig } from '../config';
...
function usage(): void {
  process.stderr
    .write(`Usage: obolus onboard --claim <code> [--wallet-name <name>] [--vault-path <path>] [--passphrase-env <ENVNAME>] [--api-base <url>]

Exchanges a one-time claim code (from the obolus dashboard) for an
api key, creates an OWS Solana wallet, and registers its address with
the backend so your operator sees live setup progress.

The raw api key is stored at ~/.obolus/config.json (chmod 0600) and
...
Options:
  ...
  --passphrase-env <ENVNAME> Name of the environment variable that holds the
                             OWS passphrase (e.g. OBOLUS_OWS_PASSPHRASE). The
                             passphrase value is read from process.env at call
                             time; only the variable NAME is persisted to
                             ~/.obolus/config.json, never the value itself.
  --api-base <url>           Override the default https://api.obolus.xyz/v1
 ...
 * running `obolus onboard` multiple times on the same machine gives
...
export async function onboardCommand(argv: string[]): Promise<number> {
  ...
  let apiBase = args.apiBase || process.env.OBOLUS_BASE_URL || 'https://api.obolus.xyz/v1';
  ...
  // key in plaintext. The escape hatch (OBOLUS_ALLOW_INSECURE_BASE_URL=1)
  // exists only for onboarding against a local backend.
  try {
    apiBase = assertSafeBaseUrl(apiBase, { context: '--api-base / OBOLUS_BASE_URL' });
  } catch (err) {
    process.stderr.write(`error: ${err instanceof Error ? err.message : String(err)}\n`);
    return 2;
  }

  // If the machine already has a Obolus config, warn before
  ...
  const existing = loadObolusConfig();
  if (existing) {
    process.stderr.write(
      '⚠ An existing obolus config was found at ~/.obolus/config.json\n' +
  ...
  try {
    ...
    if (!res.ok) {
      ...
      process.stderr.write(
        'If the code is expired, ask your operator to mint a new one in the obolus dashboard.\n',
      );
      return 1;
    }
    ...
    if (typeof data.api_key !== 'string' || !data.api_key) {
      process.stderr.write(
        'error: claim succeeded but the response is missing api_key — aborting.\n' +
          'This is a backend bug. Report it to api@obolus.xyz with the response.\n',
      );
      return 1;
    }
    ...
    // to ~/.obolus/config.json and then crash every subsequent SDK
    // call via Node's ERR_INVALID_CHAR on the X-Api-Key header. The
    // load-time check in config.ts::loadObolusConfig catches it on
    ...
    if (typeof data.api_key_id !== 'string' || !data.api_key_id) {
      process.stderr.write(
        'error: claim response missing api_key_id — aborting. Report to api@obolus.xyz.\n',
      );
      return 1;
    }
    ...
    // go to the attacker. Persisting the poisoned url to ~/.obolus/
    ...
    if (data.api_url !== undefined && data.api_url !== null) {
      ...
      if (claimUrl.protocol !== 'https:' && process.env.OBOLUS_ALLOW_INSECURE_BASE_URL !== '1') {
        process.stderr.write(
  ...
  // is persisted to config; the value never touches disk in obolus.
  ...
  const { path: configPath } = saveObolusConfig({
  ...
  process.stdout.write('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  process.stdout.write(' obolus agent ready\n');
  process.stdout.write('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  process.stdout.write(`  Label:           ${claimResponse.label ?? '(none)'}\n`);
  process.stdout.write(`  Solana address: ${publicKey}\n`);
  process.stdout.write(`  SOL balance:     ${balance.sol}\n`);
  process.stdout.write(`  USDC balance:    ${balance.usdc}\n`);
  process.stdout.write('\n');

  if (isFunded) {
    // Wallet is already activated + has enough SOL for the base
    ...
    process.stdout.write('\n');
    if (usdcNum === 0) {
      // Funded with SOL but no USDC yet. If the agent intends to be
    ...
    process.stdout.write('    npx -y obolus@latest purchase --amount 0.01\n');
    if (usdcNum > 0) {
      process.stdout.write('\n');
      process.stdout.write(`  (USDC balance ${balance.usdc} will be auto-picked for USDC-paid\n`);
      process.stdout.write('   purchases up to that amount; SOL for anything larger.)\n');
    }
  } else {
    process.stdout.write('Next step: fund the wallet.\n');
    process.stdout.write('\n');
    process.stdout.write('  Step 1 — send SOL to the Solana address above:\n');
    process.stdout.write('    • At least 2 SOL — 1 SOL minimum account balance (2 × 0.5\n');
    process.stdout.write('      base reserve) + 0.5 SOL token account subentry + ~0.5 headroom.\n');
    process.stdout.write('\n');
    process.stdout.write('  Step 2 — open the USDC token account before any USDC payment:\n');
    process.stdout.write('    npx -y obolus@latest wallet token account\n');
    process.stdout.write('\n');
    process.stdout.write('    USDC is an ISSUED asset on Solana — the holder account must\n');
    process.stdout.write('    authorize the issuer before it can hold any balance. A USDC\n');
    process.stdout.write('    payment sent to a wallet with no token account BOUNCES back to\n');
    ...
    process.stdout.write('    in SOL — SOL is native and needs no token account.)\n');
    process.stdout.write('\n');
    process.stdout.write('  Step 3 — ask the operator to send USDC (or more SOL).\n');
    process.stdout.write('\n');
    process.stdout.write('  Step 4 — buy a card:\n');
    process.stdout.write('    npx -y obolus@latest purchase --amount <USD>\n');
  }
  process.stdout.write('\n');
  process.stdout.write('Your operator sees setup progress live in the obolus dashboard.\n');
  process.stdout.write('\n');
  return 0;
}
