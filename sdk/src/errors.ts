/**
 * Structured error types for the obolus SDK.
 *
...
/** Base class — all obolus errors extend this. */
export class ObolusError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = 'ObolusError';
    Object.setPrototypeOf(this, new.target.prototype);
  }
}

/** The API key's spend limit has been reached. */
export class SpendLimitError extends ObolusError {
  constructor(
 ...
 */
export class RateLimitError extends ObolusError {
...
export class ServiceUnavailableError extends ObolusError {
...
/** SOL price feed is unavailable — retry or use USDC. */
export class PriceUnavailableError extends ObolusError {
  constructor(
    message = 'SOL price is temporarily unavailable. Retry shortly, or use payment_asset: "usdc".',
  ) {
 ...
 */
export class InvalidAmountError extends ObolusError {
  constructor(
...
export class AuthError extends ObolusError {
  constructor() {
    super(
      'Invalid or missing API key. Pass it as the X-Api-Key header, or set OBOLUS_API_KEY.',
      'invalid_api_key',
      401,
    );
    this.name = 'AuthError';
...
export class OrderFailedError extends ObolusError {
  constructor(
    public readonly orderId: string,
    reason: string,
    public readonly refund?: { solana_txid: string },
  ) {
    const refundNote = refund
      ? ` Your payment is being refunded (txid: ${refund.solana_txid}).`
 ...
 * but the flow couldn't finish — e.g. Solana RPC finalization timed out,
 * or waitForCard hit its deadline. Always carries the orderId so the caller
 * can resume via `obolus purchase --resume <order-id>` without minting a
 ...
 * never went out and resume may need to retry the Solana submit.
 */
export class ResumableError extends ObolusError {
  constructor(
    public readonly orderId: string,
    reason: string,
    ...
    super(
      `Purchase could not finish for order ${orderId}: ${reason}${hashNote}. ` +
        `Resume with: obolus purchase --resume ${orderId}`,
      'resumable',
      0,
...
export class WaitTimeoutError extends ObolusError {
  constructor(
    public readonly orderId: string,
    timeoutMs: number,
  ) {
 ...
 * Falls back to generic ObolusError for unknown codes.
 */
export function parseApiError(status: number, body: Record<string, unknown>): ObolusError {
  ...
  switch (code) {
    ...
    default:
      return new ObolusError(message, code, status, body);
  }
}
