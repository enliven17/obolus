...
function client() {
  return new ObolusClient({ baseUrl: 'http://localhost:3000/v1', apiKey: 'obolus_test_key' });
}
...
const ORDER_RESPONSE = {
  ...
  payment: {
    type: 'solana_program' as const,
    contract_id: 'OBOLUSCONTRACTIDXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX',
    order_id: 'ord_abc',
    usdc: {
      amount: '10.00',
...
describe('ObolusClient.createOrder', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('creates a USDC order and returns response', async () => {
    ...
    expect(res.payment.type).toBe('solana_program');
    expect(res.payment.contract_id).toBe(
      'OBOLUSCONTRACTIDXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX',
    );
  ...
  it('sends correct headers and body', async () => {
    ...
    expect((opts.headers as Record<string, string>)['X-Api-Key']).toBe('obolus_test_key');
  ...
  it('strips trailing slash from baseUrl', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValue({ ok: true, status: 201, json: async () => ORDER_RESPONSE });
    global.fetch = fetchMock;
    const c = new ObolusClient({ baseUrl: 'http://localhost:3000/v1/', apiKey: 'k' });
...
describe('ObolusClient.getOrder', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
...
describe('ObolusClient.waitForCard', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    vi.useFakeTimers();
  });
...
describe('ObolusClient.listOrders', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
...
describe('ObolusClient.getUsage', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
