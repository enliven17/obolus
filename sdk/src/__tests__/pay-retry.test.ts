...
// real Solana RPC. We assert what arguments each retry's build
// call receives.
...
function makePayment(): PaymentInstructions {
  return {
    ...
    sol: { amount: '1.5000000' },
  } as PaymentInstructions;
}

function makeDroppedError(txHash: string): Error & { txHash: string; dropped: true } {
  const err = new Error(
    `Solana transaction ${txHash} was accepted by the RPC but never applied on the ledger within 120s — the network rejected it pre-apply.`,
...
function makeDeps(): {
  ...
  return {
    deps: {
      ...
      >,
      submitSolanaTx: submitSpy as unknown as NonNullable<
        PayViaContractOwsDeps['submitSolanaTx']
      >,
      owsSignTx: signSpy,
...
describe('payViaContractOWS — retry on dropped Solana tx', () => {
  it('retries with preserved sequence after one dropped attempt, then succeeds', async () => {
    ...
    const promise = payViaContractOWS(
      { walletName: 'fake-wallet', payment: makePayment(), paymentAsset: 'sol' },
      deps,
    );
  ...
  it('does NOT retry on an on-chain failure (sequence already consumed)', async () => {
    ...
    submitSpy.mockRejectedValueOnce(new Error('Solana transaction HASH failed on-chain'));
  ...
  it('does NOT retry on a txHash-attached-but-not-dropped error', async () => {
    ...
    // Solana RPC-unreachable case: hash attached, dropped flag NOT set.
    // The tx may still be pending, so retry with same sequence
    // could race with a late-landing original.
    const err = new Error('Solana RPC unreachable') as Error & { txHash: string };
    ...
    ).rejects.toThrow(/Solana RPC unreachable/);
