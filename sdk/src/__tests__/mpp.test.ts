...
describe('mppCharge — happy path', () => {
  it('runs the 402 → pay → 200 sequence and returns card details', async () => {
    ...
    const result = await mppCharge({
      url: 'https://api.obolus.test/v1/cards/visa/10.00',
    ...
    expect(calls[1].headers.Authorization).toMatch(/^Payment scheme="solana"/);
...
describe('mppCharge — async 202 path', () => {
  it('polls the receipt URL until 200', async () => {
    ...
    expect(result.receiptUrl).toBe('https://api.obolus.test/v1/mpp/receipts/mpp_r_abc');
  });
...
describe('mppCharge — errors', () => {
  ...
  it('rejects non-solana methods', async () => {
    const { fetch: fetchStub } = buildFetchStub([
      () => ({
        status: 402,
        body: mockChallengeBody({
    ...
    ).rejects.toThrow(/only solana_program solana methods supported/);
  });
...
describe('mppCharge — url resolution', () => {
  it('builds the URL from baseUrl + amountUsdc', async () => {
    ...
    await mppCharge({
      baseUrl: 'https://api.obolus.test/v1/',
      amountUsdc: '10.00',
      walletName: 'w',
      _deps: { fetch: fetchStub, payViaContractOWS: stubPay as never },
    });

    expect(calls[0].url).toBe('https://api.obolus.test/v1/cards/visa/10.00');
  });
});
