// Unit tests for the CLI `obolus purchase --resume` state machine.
//
...
// waitForCard hang on a dropped Solana tx.
//
// Three layers tested:
//   1. saveLastOrder / loadLastOrder round-trip (JSON form)
//   2. Legacy bare-string format is accepted on load
//   3. purchaseCardOWS resume branch with a txHash + checkSolanaTxLanded
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'obolus-resume-test-'));
  origConfigDir = process.env.OBOLUS_CONFIG_DIR;
  process.env.OBOLUS_CONFIG_DIR = tmpDir;
});

afterEach(() => {
  if (origConfigDir === undefined) delete process.env.OBOLUS_CONFIG_DIR;
  else process.env.OBOLUS_CONFIG_DIR = origConfigDir;
  try {
...
//
// This suite spies on checkSolanaTxLanded + ObolusClient to drive
...
//
// We can't hit the real Solana network in unit tests, so we mock
...
describe('purchaseCardOWS resume state machine — F1-resume', () => {
  ...
  async function loadPurchaseCardOWSWithMocks(opts: {
    ...
    // ows.ts imports checkSolanaTxLanded from its own module scope,
    ...
    vi.spyOn(ows, 'checkSolanaTxLanded').mockImplementation(checkLandedSpy);
  ...
  //   - checkSolanaTxLanded is consulted when priorTxHash is present
  ...
  // via the integration tests that hit the real solana-web3.js.

  it('calls checkSolanaTxLanded when resume carries a priorTxHash', async () => {
    ...
    // checkSolanaTxLanded. We test the short-circuit here and the
