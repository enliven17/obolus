...
describe('ObolusError', () => {
  it('is an instance of Error', () => {
    const err = new ObolusError('msg', 'code', 400);
    expect(err).toBeInstanceOf(Error);
    expect(err).toBeInstanceOf(ObolusError);
  });

  it('sets message, code, status', () => {
    const err = new ObolusError('something broke', 'test_code', 422);
    expect(err.message).toBe('something broke');
    expect(err.code).toBe('test_code');
    expect(err.status).toBe(422);
    expect(err.name).toBe('ObolusError');
  });

  it('stores raw payload', () => {
    const raw = { detail: 'extra' };
    const err = new ObolusError('msg', 'code', 400, raw);
    expect(err.raw).toBe(raw);
  });
});

describe('SpendLimitError', () => {
  it('is instanceof ObolusError and SpendLimitError', () => {
...
describe('OrderFailedError', () => {
  ...
  it('includes refund txid in message when provided', () => {
    const err = new OrderFailedError('ord_456', 'failed', { solana_txid: 'abc123' });
    expect(err.refund).toEqual({ solana_txid: 'abc123' });
  ...
  it('is instanceof ObolusError', () => {
    expect(new OrderFailedError('x', 'y')).toBeInstanceOf(ObolusError);
  });
});
...
describe('WaitTimeoutError', () => {
  ...
  it('is instanceof ObolusError', () => {
    expect(new WaitTimeoutError('x', 1000)).toBeInstanceOf(ObolusError);
  });
});
...
describe('parseApiError', () => {
  ...
  it('returns generic ObolusError for unknown codes', () => {
