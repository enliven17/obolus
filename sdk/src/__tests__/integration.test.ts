/**
 * SDK integration test — exercises ObolusClient's HTTP serialisation
 * against the OpenAPI contract shapes by stubbing global fetch. No
 * Solana, no VCC, no CTX.
 */
...
import type { ObolusClient as ObolusClientType } from '../client';

describe('SDK integration (sandbox mode)', () => {
  let ObolusClient: typeof ObolusClientType;

  beforeAll(async () => {
    const mod = await import('../client');
    ObolusClient = mod.ObolusClient;
  });

  it('ObolusClient constructor validates API key', () => {
    expect(() => new ObolusClient({ apiKey: '' })).toThrow();
    expect(() => new ObolusClient({ apiKey: '  ' })).toThrow();
    const client = new ObolusClient({ apiKey: 'test_key', baseUrl: 'http://localhost:1234/v1' });
    expect(client).toBeDefined();
  });

  it('ObolusClient has all expected methods', () => {
    const client = new ObolusClient({ apiKey: 'test_key' });
  ...
  it('createOrder sends correct request shape', async () => {
    ...
    try {
      ...
      expect(capturedHeaders['X-Api-Key']).toBe('obolus_testkey');
      ...
      expect(result.payment.type).toBe('solana_program');
  ...
  it('getOrder sends correct request and parses response', async () => {
    ...
    globalThis.fetch = (async (url: string | URL, init?: RequestInit) => {
      ...
      expect(headers['X-Api-Key']).toBe('obolus_testkey');
      return new Response(
        JSON.stringify({
          order_id: 'order-123',
          status: 'delivered',
