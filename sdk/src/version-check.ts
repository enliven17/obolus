...
// locally-installed obolus is older than the current `latest` tag on
// the npm registry.
//
...
//     to `~/.obolus/version-check.json` and skip the registry entirely
...
const REGISTRY_URL = 'https://registry.npmjs.org/obolus/latest';
const CHECK_INTERVAL_MS = 24 * 60 * 60 * 1000; // once per day
const FETCH_TIMEOUT_MS = 2_000;
const STATE_FILE = path.join(os.homedir(), '.obolus', 'version-check.json');
...
// obolus. A hostile server (DNS hijack, MITM, compromised CDN) could
...
function printWarning(local: string, latest: string): void {
  ...
  const msg = [
    '',
    `⚠  obolus ${local} is out of date — ${latest} is available on npm.`,
    `   Upgrade: npx -y obolus@latest <command>  |  npm i -g obolus@latest`,
    `   Release notes: https://obolus.xyz/changelog`,
    '',
  ].join('\n');
  try {
