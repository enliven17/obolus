// Agent-local config file. Persisted at ~/.obolus/config.json after
// a successful `obolus onboard --claim` so the SDK can load the api
...
function defaultConfigDir(): string {
  return process.env.OBOLUS_CONFIG_DIR || path.join(os.homedir(), '.obolus');
}
...
// mints keys as `obolus_<48 hex>`, which is pure ASCII alnum + underscore).
const API_KEY_SHAPE = /^[\x20-\x7e]+$/;

export function loadObolusConfig(configPath?: string): ObolusConfig | null {
  ...
  try {
    ...
    if (stat.isSymbolicLink()) {
      throw new Error(
        `obolus config at ${p} is a symbolic link. Refusing to load. ` +
          `Remove the link and re-run 'obolus onboard --claim <code>' to create a real file.`,
      );
    }
    if (!stat.isFile()) {
      throw new Error(
        `obolus config at ${p} is not a regular file. ` +
          `Remove it and re-run 'obolus onboard --claim <code>'.`,
      );
    }
    ...
    if (stat.size > MAX_CONFIG_BYTES) {
      throw new Error(
        `obolus config at ${p} is ${stat.size} bytes (max ${MAX_CONFIG_BYTES}). ` +
          `Refusing to load — the file is either corrupted or has been tampered with. ` +
          `Rotate your api key via the dashboard and re-run 'obolus onboard'.`,
      );
    }
    ...
    if (process.platform !== 'win32' && (stat.mode & 0o077) !== 0) {
      try {
        ...
        process.stderr.write(
          `⚠ obolus config at ${p} had loose permissions (${(stat.mode & 0o777).toString(8)}) — tightened to 600.\n` +
    ...
    const config = JSON.parse(raw) as ObolusConfig;
    ...
    if (typeof config.api_key === 'string' && !API_KEY_SHAPE.test(config.api_key)) {
      throw new Error(
        `obolus config at ${p} contains an api_key with non-printable characters. ` +
          `The file may be corrupted or tampered with. Rotate your key and re-run 'obolus onboard'.`,
      );
    }
 ...
 * the explicit OBOLUS_ALLOW_INSECURE_BASE_URL escape hatch is set,
 ...
 *     OBOLUS_BASE_URL to an attacker target)
 */
export function assertSafeBaseUrl(url: string, opts: { context?: string } = {}): string {
  ...
  // F4-config: reject embedded userinfo. `https://api.obolus.xyz/v1@evil.com/`
  // parses as username='api.obolus.xyz/v1', password='', hostname='evil.com'
  // — the whole string looks plausibly obolus-ish in log output, but every
  // request would go to evil.com carrying the user's api_key in the
  // Authorization header. There's no legitimate reason for a obolus base
  ...
  if (parsed.protocol !== 'https:') {
    if (process.env.OBOLUS_ALLOW_INSECURE_BASE_URL === '1') {
      return parsed.toString();
    }
    throw new Error(
      `Refusing to use non-HTTPS base URL (${url})${opts.context ? ` for ${opts.context}` : ''}. ` +
        `Set OBOLUS_ALLOW_INSECURE_BASE_URL=1 to override for local development.`,
    );
  }
  return parsed.toString();
}
 ...
 * truncated new one that loadObolusConfig would explode on.
 *
 ...
 */
export function saveObolusConfig(config: ObolusConfig, configPath?: string): { path: string } {
  const p = configPath || defaultConfigPath();
  const dir = path.dirname(p);
  fs.mkdirSync(dir, { recursive: true, mode: 0o700 });
  // F2-config: mkdirSync's mode option only applies to directories
  // it actually CREATES. An existing ~/.obolus directory at 0755
  ...
  try {
    ...
    // rename commits. A leaked ~/.obolus/config.json.tmp-* file
...
export function resolveCredentials(
  ...
  if (!apiKey && process.env.OBOLUS_API_KEY) apiKey = process.env.OBOLUS_API_KEY;
  if (!baseUrl && process.env.OBOLUS_BASE_URL) baseUrl = process.env.OBOLUS_BASE_URL;
  ...
  // who tricks the user into setting OBOLUS_BASE_URL=http://evil/
