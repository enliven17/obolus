...
// deployment. This is the view the obolus operator gets when they
...
// matches OBOLUS_PLATFORM_OWNER_EMAIL). NO requireDashboard — the
...
// which is separately gated by @obolus.com domain + requireCardReveal.
...
// Response-size cap on Solana RPC fetches. An Solana RPC account response
...
function isSolanaRpcBodyTooBig(res) {
  const len = parseInt(res.headers.get('content-length') || '0', 10);
  return Number.isFinite(len) && len > HORIZON_RESPONSE_MAX_BYTES;
}

// Live treasury balances via Solana RPC. Best-effort — if Solana RPC is
// unreachable we return nulls so /overview still loads.
async function fetchTreasuryBalance() {
  const { Keypair } = require('@solana/web3.js');
  const secret = process.env.SOLANA_AGENT_SECRET;
  if (!secret)
    return { public_key: null, sol: null, usdc: null, error: 'SOLANA_AGENT_SECRET not set' };
  const publicKey = Keypair.fromSecret(secret).publicKey();
  const horizon =
    (process.env.SOLANA_NETWORK || 'mainnet') === 'mainnet'
  ...
  try {
    ...
    if (!res.ok) {
      return { public_key: publicKey, sol: null, usdc: null, error: `horizon http ${res.status}` };
    }
    if (isSolanaRpcBodyTooBig(res)) {
      return {
    ...
    const sol = balances.find((b) => b.asset_type === 'native')?.balance ?? '0';
    const usdc =
      balances.find((b) => b.asset_type === 'credit_alphanum4' && b.asset_code === 'USDC')
        ?.balance ?? '0';
    return { public_key: publicKey, sol, usdc, error: null };
  } catch (err) {
    return {
      public_key: publicKey,
      sol: null,
...
router.get('/overview', async (req, res) => {
  ...
  watcher.age_seconds = watcher.last_sig_at
    ? Math.round((Date.now() - new Date(watcher.last_sig_at).getTime()) / 1000)
    : null;
...
router.get('/orders', async (req, res) => {
  ...
  let query = `
    SELECT o.id, o.status, o.amount_usdc, o.payment_asset, o.solana_txid,
           o.sender_address, o.refund_solana_txid, o.card_brand,
...
// ── GET /treasury ─────────────────────────────────────────────────────────────
// Live Solana RPC balance + recent 20 outflows from the treasury account.
router.get('/treasury', async (req, res) => {
  ...
  if (balance.public_key) {
    ...
    try {
      ...
      if (r.ok && !isSolanaRpcBodyTooBig(r)) {
...
//   sensitive mutation in obolus already requires justification
