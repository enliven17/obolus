...
//      order rows.
//
// All async work (assertSafeUrl, usdToSol) happens BEFORE the txn; the
...
router.post('/', orderCreateLimiter, async (req, res) => {
  ...
  // usdToSol is an RPC fetch; we resolve it up-front and then let
  // the synchronous transaction below decide what to do with the
  // result. If the price oracle is down we simply don't advertise a
  // SOL payment branch for this order.
  let solAmount = null;
  try {
    solAmount = await usdToSol(String(amount));
  } catch (err) {
    console.warn(`[orders] SOL price lookup failed: ${err.message}`);
  }

  // Stable USDC issuer for the Solana payment envelope.
  const USDC_ISSUER =
  ...
  const txnResult = db.transaction(() => {
    ...
    // Real mode — real Solana contract payment instructions.
...
function buildOrderResponse(order) {
  ...
  if (['failed', 'refund_pending', 'refunded'].includes(order.status)) {
    ...
    if (order.status === 'refunded') {
      response.refund = { solana_txid: order.refund_solana_txid };
    }
  }
