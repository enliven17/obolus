...
router.post('/login', loginLimiter, async (req, res) => {
  ...
  // Dev: print the code to console so you can login without SMTP.
...
router.post('/verify', verifyLimiter, (req, res) => {
  ...
  res.json({
    ...
    user: {
      ...
      email: user.email,
      role: user.role,
      // Platform-owner is a deployment-level attribute (OBOLUS_PLATFORM_OWNER_EMAIL).
...
// ── POST /auth/privy ─────────────────────────────────────────────────────────
// Privy-authenticated login. Frontend sends { accessToken, email } after
// Privy's login modal completes. We verify the token against Privy's API,
// then create/find the user and issue a session cookie — same as /auth/verify.
//
// No OTP needed — Privy already handled identity verification.
...
router.post('/privy', privyLimiter, async (req, res) => {
  ...
  // Verify the Privy access token — if PRIVY_APP_ID is not set, skip
  // verification in dev (trust the email). In production, always verify.
  const privyAppId = process.env.PRIVY_APP_ID;
  const privyAppSecret = process.env.PRIVY_APP_SECRET;

  // Always verify when PRIVY_APP_ID is set (dev + prod)
  if (privyAppId) {
    try {
      ...
      // App Secret authentication adds extra security
      if (privyAppSecret) {
    ...
    // No Privy configured — reject, don't allow unauthenticated access
    return res.status(503).json({ error: 'privy_not_configured', message: 'Set PRIVY_APP_ID in backend .env' });
  }

  const now = new Date().toISOString();

  // Reuse userBootstrap pattern from /auth/verify
  const userBootstrap = db.transaction((email) => {
    let u = /** @type {any} */ (db.prepare(`SELECT * FROM users WHERE email = ?`).get(email));
    if (!u) {
      const uid = uuidv4();
      const isFirst = (/** @type {any} */ (db.prepare(`SELECT COUNT(*) AS n FROM users`).get())).n === 0;
      db.prepare(
        `INSERT INTO users (id, email, role, last_login_at) VALUES (?, ?, ?, ?)`,
      ).run(uid, email, isFirst ? 'owner' : 'user', now);
      u = /** @type {any} */ (db.prepare(`SELECT * FROM users WHERE id = ?`).get(uid));
    } else {
      db.prepare(`UPDATE users SET last_login_at = ? WHERE id = ?`).run(now, u.id);
    }
    let d = /** @type {any} */ (db.prepare(`SELECT * FROM dashboards WHERE user_id = ?`).get(u.id));
    if (!d) {
      const dashId = uuidv4();
  ...
  const { user, dashboard } = userBootstrap(addr);
  ...
  db.prepare(`INSERT INTO sessions (id, user_id, token_hash, expires_at) VALUES (?, ?, ?, ?)`).run(
    uuidv4(), user.id, hashToken(rawToken), sessionExpiresAt,
  );
