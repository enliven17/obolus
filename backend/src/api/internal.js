// @ts-check
// Internal API — obolus ops use only (admin.obolus.com)
// Protected by requireAuth + requireInternal (must be @obolus.com or in INTERNAL_EMAILS).
//
...
// @obolus.com email — making every corporate inbox a potential card
// exfiltration vector.
...
router.get('/orders', (req, res) => {
  ...
  let query = `
    SELECT o.id, o.status, o.amount_usdc, o.payment_asset,
           o.solana_txid, o.sender_address, o.refund_solana_txid,
           o.card_brand,
...
// Derives public key from SOLANA_AGENT_SECRET (legacy env name retained for
// backward compatibility); balance is fetched by the frontend from the Solana RPC.
router.get('/platform-wallet', (req, res) => {
  try {
    const { Keypair } = require('@solana/web3.js');
    const secret = process.env.SOLANA_AGENT_SECRET;
    if (!secret) return res.status(503).json({ error: 'SOLANA_AGENT_SECRET not configured' });
...
// guarded only by requireInternal (any @obolus.com mailbox) and
