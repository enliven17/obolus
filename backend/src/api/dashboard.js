...
const { usdToSol } = require('../payments/sol-price');
...
router.post('/api-keys', requirePermission('agent:create'), async (req, res) => {
  ...
  const id = uuidv4();
  const rawKey = `obolus_${crypto.randomBytes(24).toString('hex')}`;
...
router.post(
  ...
  `,
    ...
    // Build the Solana receiver-contract payment instructions for the agent.
    // VCC is not contacted until the Solana watcher sees the agent's payment —
    // so there's no vccJobId yet; that's assigned later in index.js handlePayment.
    let solAmount = null;
    try {
      solAmount = await usdToSol(String(approval.amount_usdc));
    } catch (err) {
      console.warn(`[dashboard] SOL price lookup failed: ${err.message}`);
    }
    const USDC_ISSUER =
      process.env.SOLANA_USDC_MINT || 'GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN';
    const contractPayment = {
      type: 'solana_program',
      contract_id: process.env.SOLANA_PROGRAM_ID,
    ...
    // usdToSol call the window is wide enough to matter). The second
...
// ── Merchants (obolus-enabled catalog) ─────────────────────────────────────
...
// flag (OBOLUS_PLATFORM_OWNER_EMAIL) so regular dashboard users never
// see treasury internals — only the email that runs the obolus instance
// can pull this. Balance is fetched client-side from the Solana RPC so
// the backend never has to touch the network on every poll.
router.get('/platform-wallet', requirePlatformOwner, (req, res) => {
  try {
    const { Keypair } = require('@solana/web3.js');
    const secret = process.env.SOLANA_AGENT_SECRET;
    if (!secret) {
      return res.status(503).json({ error: 'SOLANA_AGENT_SECRET not configured' });
    }
