// @ts-check
// VCC client — dispatches fulfillment jobs to the VCC service (vcc service).
...
// Audit C-7: simple circuit breaker on the obolus → vcc path. If vcc is
...
async function getVccToken() {
  const stored = /** @type {{ value: string } | undefined} */ (
    await db.prepare(`SELECT value FROM system_state WHERE key = 'vcc_token'`).get()
  )?.value;
  if (stored) {
    try {
      ...
      await db.prepare(`DELETE FROM system_state WHERE key = 'vcc_token'`).run();
  ...
  const label = process.env.VCC_INSTANCE_LABEL || `obolus-${process.env.NODE_ENV || 'prod'}`;
  let res;
  try {
...
// Asks VCC to create a virtual card order. Returns { vccJobId, paymentUrl }
// where paymentUrl is a web+solana:pay URI that obolus must pay directly.
//
...
async function getInvoice(orderId, amountUsdc, requestId = null, callbackNonce = null) {
  ...
  const callbackBase =
    process.env.OBOLUS_BASE_URL || `http://localhost:${process.env.PORT || 4000}`;
  ...
  const existingRow = await db
    .prepare(`SELECT callback_secret FROM orders WHERE id = ?`)
    .get(orderId);
  let callbackSecret;
  if (existingRow?.callback_secret) {
    try {
      ...
      await db
        .prepare(`UPDATE orders SET callback_secret = ? WHERE id = ?`)
        .run(seal(callbackSecret), orderId);
    }
  } else {
    callbackSecret = crypto.randomBytes(32).toString('hex');
    await db
  ...
  // Audit C-3: per-job nonce. Generated by obolus at invoice time, stored
  ...
  // payment_url is the Solana URI that obolus pays next — a malformed
  ...
  // expect, and require payment_url to be a solana: / web+solana: URI
  ...
  if (!/^(web\+)?solana:|^vcc:/i.test(body.payment_url)) {
...
// Tells VCC that payment confirmed. VCC transitions the job to queued and
