...
//
// Order of operations:
//   1. Stop the Solana watcher from scheduling new polls. Any
...
function gracefulShutdown(signal) {
  ...
  shuttingDown = true;
  console.log(`[obolus] ${signal} — draining and shutting down`);
  try {
    stopWatcher?.();
  } catch (err) {
    console.error('[obolus] stopWatcher error:', err);
  }
  try {
    stopJobs();
  } catch (err) {
  ...
  setTimeout(() => {
    console.error('[obolus] shutdown timeout — forcing exit');
    process.exit(1);
  }, 15_000).unref();
}
...
process.on('uncaughtException', (err, origin) => {
  console.error(`[obolus] uncaughtException at ${origin}:`, err);
  try {
...
process.on('unhandledRejection', (reason) => {
  ...
  console.error(
    `[obolus] unhandledRejection: ${payload.name}: ${payload.message}${
      payload.stack ? `\n${payload.stack}` : ''
    }`,
  );
  try {
...
// from the individual modules (./app, ./jobs, ./payments/solana,
