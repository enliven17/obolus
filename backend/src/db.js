...
const DB_PATH = process.env.DB_PATH || path.join(__dirname, '..', 'obolus.db');
...
applyMigration(1, () => {
  for (const sql of [
    ...
    `ALTER TABLE orders ADD COLUMN payment_sol_amount TEXT`,
    `ALTER TABLE orders ADD COLUMN sender_address TEXT`,
    `ALTER TABLE orders ADD COLUMN refund_solana_txid TEXT`,
...
// Migration 10: stuck-ordering recovery — track whether the CTX SOL payment has
...
applyMigration(10, () => {
  for (const sql of [
    `ALTER TABLE orders ADD COLUMN sol_sent_at TEXT`,
  ...
  // treat every such order as "sol_sent_at IS NULL" and retry payCtxOrder,
  ...
  // getInvoice, and since handlePayment sends SOL immediately after storing
  // vcc_job_id, we assume both sol_sent_at and vcc_notified_at should be
...
//
// The agent runs `npx obolus onboard --claim <code>`. The CLI hits
...
// config file (~/.obolus/config.json) and the SDK loads from there.
//
...
applyMigration(15, () => {
  db.exec(`
    CREATE TABLE IF NOT EXISTS agent_claims (
      ...
      api_key_id        TEXT NOT NULL,
      -- Sealed via lib/secret-box (AES-256-GCM with OBOLUS_SECRET_BOX_KEY).
      -- Contains the raw 'obolus_...' api key so the claim endpoint can
...
// expected_sol_amount is the SOL we quoted to the agent at order-creation
// time for the pay_sol path (kept under its legacy SOL column name to avoid
// a breaking schema change). handlePayment compares the on-chain event
// amount against these values and rejects mismatches to unmatched_payments
// so the treasury can't be drained by a tiny pay_sol/pay_usdc event
// against a real pending_payment order (adversarial audit finding F0).
applyMigration(20, () => {
  for (const sql of [`ALTER TABLE orders ADD COLUMN expected_sol_amount TEXT`]) {
    try {
      db.prepare(sql).run();
    } catch (_) {
      /* column already exists */
...
// Migration 23: solana watcher dead-letter table. Before this, any
...
//
// Migration 31 below renames this table to solana_dead_letter; both
// CREATE statements remain so a fresh DB lands on the new name directly
// while existing DBs flow through the rename.
applyMigration(23, () => {
  db.exec(`
...
// Migration 25: ctx_solana_txid column. Adversarial audit F1-jobs
// (2026-04-15) — outbound CTX payments that throw with
// solanaStatus='unknown' or 'applied_failed' (the tx MAY have landed
...
// reconciler's retry query filters `ctx_solana_txid IS NULL` so a
...
applyMigration(25, () => {
  ...
  try {
    db.exec(`ALTER TABLE orders ADD COLUMN ctx_solana_txid TEXT`);
  } catch (err) {
...
// invoice amount (in SOL, kept under the legacy "sol" column name to
// avoid a breaking schema change) and the SOL/USD rate at settlement
// time so the platform dashboard can compute real per-order margins
// instead of estimating from a hardcoded discount percentage. The
// effective discount varies by upstream provider and product;
// capturing the actual numbers means margin reporting automatically
// reflects changes in CTX's pricing without a code deploy.
applyMigration(26, () => {
  for (const sql of [
    // The SOL amount CTX invoiced for this order, parsed from the
    // payment_url. NULL for pre-migration orders (use estimate).
    `ALTER TABLE orders ADD COLUMN ctx_invoice_xlm TEXT`,
    // SOL/USD spot rate at the time we paid CTX. Lets the dashboard
...
//     GET /v1/cards/:product/:amount with the Solana tx that pays for it.
...
// Migration 29: store the quoted SOL amount on the challenge row.
// Without this, the retry handler re-quotes SOL at verification time,
...
applyMigration(29, () => {
  try {
    db.prepare(`ALTER TABLE mpp_challenges ADD COLUMN amount_sol TEXT`).run();
  } catch (err) {
    if (!/duplicate column name/.test(/** @type {Error} */ (err)?.message || '')) throw err;
  }
});

// Migration 30: dashboards.updated_at column for audit tracking.
applyMigration(30, () => {
  try {
    db.prepare(`ALTER TABLE dashboards ADD COLUMN updated_at TEXT`).run();
  } catch (err) {
    if (!/duplicate column name/.test(/** @type {Error} */ (err)?.message || '')) throw err;
  }
});

// Migration 31: rename solana_dead_letter -> solana_dead_letter as part of
// the Solana -> Solana platform migration. Existing databases created
// under migration 23's old name flow through this rename; fresh databases
// created on this binary land on the new name directly because the
// CREATE TABLE in migration 23 has been updated to the new name (which
// makes the rename here a no-op on fresh DBs — the table simply doesn't
// exist under the old name).
applyMigration(31, () => {
  // Detect old table presence; SQLite doesn't have ALTER TABLE IF EXISTS.
  const oldTable = /** @type {any} */ (
    db
      .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='solana_dead_letter'`)
      .get()
  );
  if (oldTable) {
    // If both tables somehow exist (edge case from out-of-order ops), drop
    // the freshly-created empty new table before rename to avoid conflict.
    const newTable = /** @type {any} */ (
      db
        .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='solana_dead_letter'`)
        .get()
    );
    if (newTable) {
      db.exec(`DROP TABLE solana_dead_letter;`);
    }
    db.exec(`ALTER TABLE solana_dead_letter RENAME TO solana_dead_letter;`);
  }
  db.exec(`DROP INDEX IF EXISTS idx_solana_dead_letter_created_at;`);
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_solana_dead_letter_created_at ON solana_dead_letter(created_at);`,
  );
});
