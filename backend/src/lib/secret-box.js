...
//   OBOLUS_SECRET_BOX_KEY  (preferred, new)
...
// validates OBOLUS_SECRET_BOX_KEY via zod, but the legacy
...
function loadKeyFromEnv() {
  const preferred = process.env.OBOLUS_SECRET_BOX_KEY;
  const legacy = process.env.VCC_TOKEN_KEY;
  for (const [name, val] of [
    ['OBOLUS_SECRET_BOX_KEY', preferred],
  ...
  if (preferred && legacy && preferred !== legacy) {
    console.warn(
      '[secret-box] both OBOLUS_SECRET_BOX_KEY and VCC_TOKEN_KEY are set ' +
        'to different values — using OBOLUS_SECRET_BOX_KEY. Unset the ' +
...
function seal(plaintext) {
  ...
  if (!key) {
    if (isProduction()) {
      throw new Error(
        'secret-box: OBOLUS_SECRET_BOX_KEY is required in production. ' +
    ...
    if (!warnedAboutMissingKey) {
      console.warn(
        '[secret-box] OBOLUS_SECRET_BOX_KEY not set — sealing as plaintext (dev/test only)',
      );
...
function open(stored) {
  ...
  if (!key) {
    throw new Error(
      'secret-box: OBOLUS_SECRET_BOX_KEY not set, cannot decrypt. Generate one with `openssl rand -hex 32` and set it in the environment.',
    );
  }
