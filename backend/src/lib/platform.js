...
//
// obolus has two distinct authorities:
//
...
//
//   2. The PLATFORM owner is whoever runs obolus itself — the email
...
//
// Without `OBOLUS_PLATFORM_OWNER_EMAIL` set, no one is treated as the
...
//     `'' === ''` which is TRUE. If OBOLUS_PLATFORM_OWNER_EMAIL was
...
function isPlatformOwner(email) {
  const configured = process.env.OBOLUS_PLATFORM_OWNER_EMAIL;
