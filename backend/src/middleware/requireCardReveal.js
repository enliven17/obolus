...
// PAN/CVV/expiry to anyone with an @obolus.com mailbox. That made every
...
// OBOLUS_CARD_REVEAL_EMAILS can pass. If the env var is unset, NOBODY can
...
function getCardRevealEmails() {
  const raw = process.env.OBOLUS_CARD_REVEAL_EMAILS || '';
  return raw
    .split(',')
...
module.exports = function requireCardReveal(req, res, next) {
  ...
  if (allowed.length === 0) {
    ...
    return res.status(403).json({
      ...
      message:
        'Raw card reveal is disabled in this deployment. Set OBOLUS_CARD_REVEAL_EMAILS to ' +
