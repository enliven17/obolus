...
// number we can hand to the Solana sender. Protects against corrupt order
// rows (null, "", "0", "-5", "abc") making it into sendUsdc/sendSol where
// the error would only surface as a cryptic Solana RPC response.
function isValidRefundAmount(amount) {
  if (amount === null || amount === undefined || amount === '') return false;
  const s = String(amount).trim();
  if (!/^\d+(\.\d+)?$/.test(s)) return false;
  // Solana precision is 7 decimal places. Ensure at least one non-zero digit.
...
//   excess_usdc  = '0.5000000'   (the overpayment delta, in lamport
//                                 precision)
...
// lamports (same precision model as payment-handler.js::compareDecimal)
 ...
 * Returns a decimal-string in lamport precision (7 fractional digits).
...
function computeUsdcRefundAmount(amountUsdc, excessUsdc) {
  const quoted = toLamportsOrZero(amountUsdc);
  const excess = toLamportsOrZero(excessUsdc);
  return lamportsToDecimal(quoted + excess);
}

// Adversarial audit F1-fulfillment (2026-04-15). solana-sender's submitWithRetry
// now annotates submit failures with `solanaStatus` ('not_landed' | 'applied_
// failed' | 'unknown') and a pre-computed `txHash` (see audit F1-solana-sender in
// the previous cycle). A refund tx that might have landed during a lost-
// response window — solanaStatus='unknown' — is ambiguous: the tx may be on
...
//
//   - If the thrown error carries a txHash (solanaStatus='unknown' or
//     'applied_failed'), write it to orders.refund_solana_txid so ops can
//     check Solana RPC directly. The order stays in refund_pending — we DO NOT
...
//     asset, amount, solanaStatus, txHash, and the error message so an
//     alerting pipeline can page on the must-verify states instead of
//     burying them in stdout.
//
// Legacy errors (no solanaStatus marker — e.g. an SDK throw we never
// touched) fall through the same code path but with solanaStatus='legacy'
...
function recordRefundSendFailure(orderId, asset, amount, err) {
  ...
  const reviewTag =
    solanaStatus === 'unknown' || solanaStatus === 'applied_failed'
      ...
      : 'SAFE_TO_RETRY';
  console.log(
    `[refund] ${orderId}: ${asset} refund failed [${solanaStatus}] [${reviewTag}] txHash=${
      txHash || 'none'
    }: ${/** @type {Error} */ (err)?.message} — remains refund_pending`,
  );
}

// Refund the payment for an order. Sends USDC or SOL back to the sender address.
...
async function scheduleRefund(orderId) {
  ...
  if (isXlm) {
    ...
    try {
      ...
      recordRefundSendFailure(orderId, 'sol', solAmount, err);
    }
  } else {
    ...
    // in lamport precision (BigInt) and returns a decimal-string total.
    ...
    // given toLamportsOrZero is tolerant), fall back to the quoted
