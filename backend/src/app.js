// @ts-check
// Express application — importable without starting the Solana watcher or jobs.
...
//      to vcc service. A garbage-shaped id breaks those fetches with
...
// list. Bump to two years + preload so api.obolus.com can be
...
// "https//obolus.com" (missing colon) otherwise silently fails
...
// path, so "https://obolus.com/" would never match anyway —
  ...
  try {
    ...
    console.error(
      `[cors] CORS_ORIGINS entry ${JSON.stringify(entry)} is not a valid URL. ` +
        `Generate one like "https://obolus.com" (no path, no trailing slash).`,
    );
    process.exit(1);
  }
}
...
app.get('/api/version', versionLimiter, (_req, res) => {
  res.json({
    ...
    version: '0.1.0',
    hmac_protocol: 'v3',
    features: ['idempotency_key', 'solana_program', 'webhook_circuit_breaker', 'callback_nonce'],
  });
});
...
app.post('/v1/agent/claim', claimLimiter, (req, res) => {
  ...
  // If open() threw — missing OBOLUS_SECRET_BOX_KEY, corrupt blob,
  ...
  try {
    ...
    // the env var name ("OBOLUS_SECRET_BOX_KEY") back to an
  ...
  res.json({
    ...
    api_key_id: row.api_key_id,
    label: key?.label ?? null,
    api_url: process.env.PUBLIC_API_BASE_URL || 'https://api.obolus.com/v1',
  });
});
...
// obolus.com/status (and status.obolus.com via the proxy rewrite).
...
// DEV ONLY: simulate Solana payment for an order (skip real on-chain tx)
if (process.env.NODE_ENV !== 'production') {
  app.post('/dev/simulate-payment/:orderId', async (req, res) => {
    ...
    handlePayment({
      txid: `devnet_sim_${Date.now()}`,
      paymentAsset: 'usdc_solana',
      amountUsdc: `${order.amount_usdc}.000000`,
      amountSol: null,
      senderAddress: 'J98R1EtNppvAFPXrviBUhFZbxoDpTCL7vjBwDRxVpKyk',
      orderId,
    }).catch((e) => console.error('[dev] simulate-payment error:', e.message));
    res.json({ ok: true, order_id: orderId, simulated: true });
  });
}
...
app.get('/status', statusLimiter, (req, res) => {
  ...
  // Solana watcher freshness. `solana_last_sig_at` is updated every time
  // the watcher successfully saves a cursor. The watcher polls every 3s
  // and backs off to 12s on errors. 120s threshold = 10× the error
  // backoff — any gap longer than that almost certainly means the watcher
  // has silently died.
  ...
  // first poll.
  const solanaWatcherStalled =
    lastLedgerAgeSeconds !== null && lastLedgerAgeSeconds > SOLANA_WATCHER_MAX_AGE_SECONDS;
  ...
  res.json({
    ok:
      ...
      solanaDeadLetter24h === 0 &&
      webhooksFailedPermanent24h < 5 &&
      !solanaWatcherStalled,
    frozen,
    ...
    solana_watcher: {
      last_signature: lastSigRow?.value || null,
      last_sig_at: lastLedgerAt,
      age_seconds: lastLedgerAgeSeconds,
      stalled: solanaWatcherStalled,
      max_age_seconds: SOLANA_WATCHER_MAX_AGE_SECONDS,
      dead_letter_24h: solanaDeadLetter24h,
    },
    webhooks: {
...
app.post('/v1/agent/status', agentStatusLimiter, (req, res) => {
  ...
  if (wallet_public_key !== undefined && wallet_public_key !== null) {
    let valid = false;
    if (typeof wallet_public_key === 'string') {
      try {
        new PublicKey(wallet_public_key);
        valid = true;
      } catch {
        valid = false;
      }
