...
// Import as a module object so tests can monkey-patch `solanaSender.payCtxOrder`
...
// hammering Solana RPC. Real DoS from a deploy mistake.
//
...
// the caller MUST NOT proceed to mark sol_sent_at (which would unpark
...
// send throws with solanaStatus in {unknown, applied_failed} AND a
// txHash is attached, this helper:
//
//   1. Persists the hash to orders.ctx_solana_txid so ops can verify
//      on-chain.
...
//      DB update (setting sol_sent_at would unpark the row on the next
//      reconciler tick).
//
...
async function callPayCtxOrderWithAmbiguousPark(paymentUrl, opts, orderId, shortId) {
  try {
    return await solanaSender.payCtxOrder(paymentUrl, opts);
  } catch (err) {
    const status = /** @type {any} */ (err)?.solanaStatus;
...
//   - payCtxOrder never ran/failed → vcc_job_id set, sol_sent_at IS NULL
//   - notifyPaid never ran         → vcc_job_id set, sol_sent_at set, vcc_notified_at IS NULL
//
// Each attempt is idempotent on the vcc side:
//   - getInvoice: UNIQUE(tenant_id, order_id) returns the existing job
//   - payCtxOrder: guarded by sol_sent_at so a successful prior send is not retried
...
async function reconcileOrderingFulfillment() {
  ...
  const stuck = /** @type {any[]} */ (
    ...
    WHERE status = 'ordering'
      AND (vcc_job_id IS NULL OR sol_sent_at IS NULL OR vcc_notified_at IS NULL)
      AND ctx_solana_txid IS NULL
      AND updated_at < ?
  `,
      )
      .all(cutoff)
  ...
  for (const order of stuck) {
    ...
    try {
      ...
      // failed after getInvoice would silently re-send as raw treasury SOL
...
// admin, which has key access; obolus must NOT attempt to read card data
...
// retention sweep on the encrypted upstream copy; this is the obolus
...
// Poll Solana RPC for every agent wallet sitting in 'awaiting_funding'.
// As soon as a wallet has enough SOL to pay the base reserve + any
...
// canonical SOLANA_USDC_MINT variable instead of hardcoding the
// mainnet issuer. Pre-fix, this function used the mainnet Circle
// issuer directly — so on a testnet deploy (SOLANA_NETWORK=testnet),
// the balance check never matched any entry in the `balances` array
// and agents who funded with USDC only (no SOL) got stuck in
// `awaiting_funding` forever. sol-sender.js already reads from the
...
// successful fetch — see recordSolanaRpcResult below.
let _horizonOutageAlerted = false;

// Solana RPC base URL picked by SOLANA_NETWORK (legacy env name retained
// for backward compatibility). Testnet deployments point at
// horizon-testnet.solana.org; mainnet at horizon.solana.org. Pre-fix
// the mainnet URL was hardcoded too, which broke every testnet deploy
// at the same point the USDC issuer bug did.
function horizonBase() {
  return process.env.SOLANA_NETWORK === 'testnet'
    ? 'https://horizon-testnet.solana.org'
    : 'https://horizon.solana.org';
}
...
async function checkAgentFundingStatus() {
  ...
  for (const row of awaiting) {
    try {
      ...
      // dead Solana RPC response blocks the entire awaiting_funding
      // loop until Node's default socket-level timeout (which is
      // effectively never). A 5s cap means a Solana RPC incident can
      ...
      if (!res.ok) {
        ...
        // Solana RPC outage made funding detection silently break for
        ...
        // Solana RPC problem without spam on every awaiting row.
        if (!_horizonOutageAlerted) {
          ...
          console.error(
            `[jobs] funding check: Solana RPC returned HTTP ${res.status} ` +
      ...
      const sol = parseFloat(xlmStr);
      const usdc = parseFloat(usdcStr);
      // Funded = at least 1 SOL past the Solana base reserve, OR any
      ...
      // false — a corrupt response leaves the wallet as awaiting,
      // not mis-flipped to funded.
      ...
      // Transient Solana RPC failure (network, timeout, JSON parse) —
...
// Test-only: reset the Solana RPC outage alert dedup state.
function _resetFundingSolanaRpcOutageState() {
  _horizonOutageAlerted = false;
}
...
// Same pattern for checkAgentFundingStatus. The per-wallet Solana RPC
...
function startJobs() {
  ...
  // Solana RPC is cheap, the query is bounded (only awaiting_funding rows),
  ...
  // treated it as 1ms → CPU spin on Solana RPC fetches.
...
module.exports = {
  ...
  _resetFundingSolanaRpcOutageState,
