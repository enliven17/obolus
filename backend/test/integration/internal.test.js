...
process.env.OBOLUS_CARD_REVEAL_EMAILS = 'test@obolus.xyz';
...
describe('GET /internal/orders/:id/card — F1 audit row shape', () => {
  ...
  it('writes the audit row with actor_role = operator role (F1), NOT a hard-coded string', async () => {
    ...
    assert.equal(auditRow.actor_email, 'test@obolus.xyz');
  ...
  it('different operator roles produce different actor_role rows (not a constant)', async () => {
    ...
    process.env.OBOLUS_CARD_REVEAL_EMAILS = 'owner-test@obolus.xyz,admin-test@obolus.xyz';

    const owner = createTestSession({ email: 'owner-test@obolus.xyz', role: 'owner' });
    const admin = createTestSession({ email: 'admin-test@obolus.xyz', role: 'admin' });
    const orderId = await seedDeliveredOrderWithCard();
    ...
    assert.equal(byEmail['owner-test@obolus.xyz'], 'owner');
    assert.equal(byEmail['admin-test@obolus.xyz'], 'admin');
  });
