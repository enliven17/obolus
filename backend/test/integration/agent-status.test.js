...
describe('POST /v1/agent/status', () => {
  ...
  //
  // The previous regex /^G[A-Z2-7]{55}$/ accepted any 56-char base58
  // string, including ones with a wrong Ed25519 checksum. Now we use
  // StrKey.isValidEd25519PublicKey which enforces the checksum.

  it('accepts a valid Solana base58 public key for wallet_public_key', async () => {
  ...
  it('rejects a base58 public key with the right shape but wrong checksum (F2)', async () => {
    // 56-char base58 string starting with G — passes the old regex
