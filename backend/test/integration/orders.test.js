...
const VCC_TEST_PAYMENT = {
  solana_address: 'J98R1EtNppvAFPXrviBUhFZbxoDpTCL7vjBwDRxVpKyk',
  memo: 'dGVzdC1tZW1vLTIy', // 22-char base64url
  usdc: { amount: '10.00', asset: 'USDC:EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v' },
  sol: { amount: '64.5161290' },
};

global.fetch = async (url) => {
  const urlStr = typeof url === 'string' ? url : url.toString();

  if (urlStr.includes('localhost:5000/api/register')) {
    return {
      ok: true,
      status: 201,
      json: async () => ({
  ...
  if (urlStr.includes('localhost:5000/api/jobs')) {
    return {
      ok: true,
      status: 202,
      json: async () => ({
  ...
  if (urlStr.includes('rates.obolus.internal')) {
    return {
      ok: true,
      status: 200,
...
describe('POST /v1/orders', () => {
  ...
  it('returns 401 with wrong API key', async () => {
    const res = await request
      .post('/v1/orders')
      .set('X-Api-Key', 'obolus_wrong_key')
      .send({ amount_usdc: '10.00' });
    assert.equal(res.status, 401);
    assert.equal(res.body.error, 'invalid_api_key');
  });

  it('creates an order and returns Solana contract payment instructions', async () => {
    const res = await request
      .post('/v1/orders')
    ...
    assert.equal(payment.type, 'solana_program', 'payment.type must be solana_program');
    assert.ok(payment.contract_id?.startsWith('C'), 'contract_id must be a Solana C-address');
    ...
    // sol quote is best-effort (depends on the stubbed usdToXlm) — allow either present or absent
...
describe('GET /v1/orders/:id', () => {
  ...
  it('releases the stream slot when initial emit throws (F1)', async () => {
    ...
    // OBOLUS_SECRET_BOX_KEY is unset, so any `enc:` blob throws
    // `secret-box: OBOLUS_SECRET_BOX_KEY not set, cannot decrypt`.
