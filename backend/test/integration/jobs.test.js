...
describe('retryWebhooks — vault failure path (F1)', () => {
  ...
  it('marks row permanently failed on card-vault decrypt failure without firing', async () => {
    ...
    // Force a real OBOLUS_SECRET_BOX_KEY so the vault actually tries
    ...
    const prev = process.env.OBOLUS_SECRET_BOX_KEY;
    process.env.OBOLUS_SECRET_BOX_KEY = 'a'.repeat(64);
    ...
    try {
      ...
      if (prev === undefined) delete process.env.OBOLUS_SECRET_BOX_KEY;
      else process.env.OBOLUS_SECRET_BOX_KEY = prev;
      delete require.cache[require.resolve('../../src/lib/secret-box')];
      delete require.cache[require.resolve('../../src/lib/card-vault')];
    }
