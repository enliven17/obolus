...
function seedOrder(overrides = {}) {
  ...
  `,
    ...
    overrides.payment_asset: 'usdc_solana'',
...
describe('POST /vcc-callback — failed', () => {
  ...
  it('transitions order to failed and queues a refund', async () => {
    const id = seedOrder({ status: 'ordering', payment_asset: 'usdc_solana'' });
    ...
    // sol/usdc send resolves in this tick) onward to refunded. Any of the
    // three terminal-fail states is acceptable — the point of the assertion
    // is that obolus moved the order off 'ordering' and recorded the
    // VCC-supplied error.
    assert.ok(
...
describe('POST /vcc-callback — F1 nonce enforcement', () => {
  ...
  it('rejects when X-VCC-Nonce header value differs from the stored nonce', async () => {
    ...
    // doesn't match what obolus stored at invoice time.
