...
const PLATFORM_OWNER_EMAIL = 'platform-owner@obolus.test';
...
describe('GET /dashboard/platform/orders — F1 query param shape', () => {
  ...
  beforeEach(() => {
    resetDb();
    origOwnerEnv = process.env.OBOLUS_PLATFORM_OWNER_EMAIL;
    process.env.OBOLUS_PLATFORM_OWNER_EMAIL = PLATFORM_OWNER_EMAIL;
  });
  afterEach(() => {
    if (origOwnerEnv === undefined) delete process.env.OBOLUS_PLATFORM_OWNER_EMAIL;
    else process.env.OBOLUS_PLATFORM_OWNER_EMAIL = origOwnerEnv;
  });
...
describe('GET /dashboard/platform/audit — F2 details parsing', () => {
  ...
  function seedAuditRow(details) {
    ...
    recordAudit({
      dashboardId: 'system',
      actor: { id: null, email: 'test-actor@obolus.xyz', role: 'owner' },
  ...
  it('falls back to the raw string on a non-JSON details column (forward-compat)', async () => {
    ...
    db.prepare(
      `INSERT INTO audit_log (dashboard_id, actor_user_id, actor_email, actor_role,
       ...
       VALUES ('system', NULL, 'legacy@obolus.xyz', 'owner',
