...
// the e2e-obolus-vcc integration test. This file covers the three
...
// ── F2-payment-handler: parseStrictPositiveLamports helper ──────────────────

describe('F2-payment-handler: parseStrictPositiveLamports', () => {
  it('accepts a simple positive integer', () => {
    assert.equal(_parseStrictPositiveLamports('1'), 10_000_000n);
  });

  it('accepts a positive decimal', () => {
    assert.equal(_parseStrictPositiveLamports('0.0123456'), 123_456n);
  });

  it('accepts "10.00"', () => {
    assert.equal(_parseStrictPositiveLamports('10.00'), 100_000_000n);
  });

  it('rejects empty string', () => {
    assert.equal(_parseStrictPositiveLamports(''), null);
  });
  ...
  it('rejects null and undefined', () => {
    assert.equal(_parseStrictPositiveLamports(null), null);
    assert.equal(_parseStrictPositiveLamports(undefined), null);
  });

  it('rejects non-string types', () => {
    // @ts-expect-error intentional
    assert.equal(_parseStrictPositiveLamports(10), null);
    // @ts-expect-error intentional
    assert.equal(_parseStrictPositiveLamports(true), null);
    // @ts-expect-error intentional
    assert.equal(_parseStrictPositiveLamports({ amount: '10' }), null);
  });

  it('rejects zero', () => {
    // The whole point of the guard — zero is the exact value that
    // pre-fix caused the treasury drain.
    assert.equal(_parseStrictPositiveLamports('0'), null);
    assert.equal(_parseStrictPositiveLamports('0.0000000'), null);
  });

  it('rejects negative', () => {
    assert.equal(_parseStrictPositiveLamports('-1'), null);
    assert.equal(_parseStrictPositiveLamports('-0.5'), null);
  });

  it('rejects garbage strings', () => {
    assert.equal(_parseStrictPositiveLamports('abc'), null);
    assert.equal(_parseStrictPositiveLamports('1.2.3'), null);
    assert.equal(_parseStrictPositiveLamports('1e5'), null);
    assert.equal(_parseStrictPositiveLamports('NaN'), null);
    assert.equal(_parseStrictPositiveLamports('Infinity'), null);
    assert.equal(_parseStrictPositiveLamports('+1'), null);
  });
});
...
describe('F2-payment-handler: corrupt order.amount_usdc is fail-closed', () => {
  ...
  function findUnmatched(txid) {
    return db.prepare(`SELECT * FROM unmatched_payments WHERE solana_txid = ?`).get(txid);
  }
  ...
  it('routes to corrupt_order when amount_usdc is "0"', async () => {
    ...
    // comparison. toLamports('0') === 0n, and any positive on-chain
    // amount compared as "overpayment" and transitioned the order to
    // 'ordering'. Post-fix: rejected at parseStrictPositiveLamports.
...
// systematically over-paying by 10% would go unnoticed. SOL had the
...
describe('F3-payment-handler: usdc_overpaid bizEvent', () => {
  ...
  it('emits payment.usdc_overpaid when the agent pays more than expected', async () => {
    ...
    try {
      ...
      // excess_usdc should be '1.5000000' (lamport-precision stringification).
