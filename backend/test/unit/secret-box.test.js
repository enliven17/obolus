...
describe('secret-box — happy path', () => {
  ...
  beforeEach(() => {
    origPreferred = process.env.OBOLUS_SECRET_BOX_KEY;
    origLegacy = process.env.VCC_TOKEN_KEY;
    process.env.OBOLUS_SECRET_BOX_KEY = GOOD_KEY;
    delete process.env.VCC_TOKEN_KEY;
  });

  afterEach(() => {
    if (origPreferred === undefined) delete process.env.OBOLUS_SECRET_BOX_KEY;
    else process.env.OBOLUS_SECRET_BOX_KEY = origPreferred;
...
describe('secret-box — strict key validation (F1)', () => {
  ...
  it('rejects a short OBOLUS_SECRET_BOX_KEY with a specific error', () => {
    delete process.env.VCC_TOKEN_KEY;
    process.env.OBOLUS_SECRET_BOX_KEY = BAD_KEY_SHORT;
    const box = freshModule();
    assert.throws(() => box.seal('x'), /OBOLUS_SECRET_BOX_KEY must be 64 hex/);
  });

  it('rejects a non-hex OBOLUS_SECRET_BOX_KEY with a specific error', () => {
    delete process.env.VCC_TOKEN_KEY;
    process.env.OBOLUS_SECRET_BOX_KEY = BAD_KEY_NONHEX;
    const box = freshModule();
    assert.throws(() => box.seal('x'), /OBOLUS_SECRET_BOX_KEY must be 64 hex/);
  });
  ...
  it('rejects a malformed VCC_TOKEN_KEY legacy fallback (regression guard)', () => {
    ...
    delete process.env.OBOLUS_SECRET_BOX_KEY;
...
describe('secret-box — split-value warning (F2)', () => {
  ...
  it('warns when both env vars are set to different values', () => {
    ...
    assert.ok(
      warnings.some((w) => /both OBOLUS_SECRET_BOX_KEY and VCC_TOKEN_KEY/.test(w)),
