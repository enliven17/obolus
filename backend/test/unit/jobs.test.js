...
describe('expireStaleOrders', () => {
  ...
  it('expires usdc_solana pending_payment order older than 2 hours', () => {
    const id = seedOrderAt({ payment_asset: 'usdc_solana'', minutesAgo: 121 });
    expireStaleOrders();
    const order = db.prepare(`SELECT status FROM orders WHERE id = ?`).get(id);
    assert.equal(order.status, 'expired');
  });

  it('does NOT expire usdc_solana order younger than 2 hours', () => {
    const id = seedOrderAt({ payment_asset: 'usdc_solana'', minutesAgo: 60 });
    expireStaleOrders();
    const order = db.prepare(`SELECT status FROM orders WHERE id = ?`).get(id);
  ...
  it('does NOT expire usdc_solana order exactly at 2 hours', () => {
    const id = seedOrderAt({ payment_asset: 'usdc_solana'', minutesAgo: 119 });
    expireStaleOrders();
    const order = db.prepare(`SELECT status FROM orders WHERE id = ?`).get(id);
    assert.equal(order.status, 'pending_payment');
  });
  ...
  it('expires multiple stale usdc_solana orders in one pass', () => {
    const ids = [
      seedOrderAt({ payment_asset: 'usdc_solana'', minutesAgo: 125 }),
      seedOrderAt({ payment_asset: 'usdc_solana'', minutesAgo: 130 }),
    ];
    expireStaleOrders();
    for (const id of ids) {
...
describe('F1-reconcile: race with concurrent delivered flip in hard-fail branch', () => {
  ...
  function seedOrderingPastFailCutoff({ attempt = 0, vcc_job_id = 'vcc-job-reconcile' } = {}) {
    ...
    db.prepare(
      ...
      VALUES (?, 'ordering', '10.00', 'usdc_solana', NULL, ?, ?, 'GRACESENDER', ?, ?)
    `,
...
// have landed during a lost-response window (solanaStatus='unknown'
...
//
//   (a) The reconciler SELECT excludes rows with ctx_solana_txid set
...
describe('reconcileOrderingFulfillment — F1 ambiguous CTX payment parking', () => {
  const { reconcileOrderingFulfillment } = require('../../src/jobs');
  const xlmSender = require('../../src/payments/sol-sender');
  const realPayCtxOrder = xlmSender.payCtxOrder;

  beforeEach(() => {
    resetDb();
    // Default: make sol-sender succeed if called so tests that aren't
    ...
    vccClient.getVccJobStatus = async () => ({
      ...
      payment_url: 'solana:pay?destination=G...',
    });
    vccClient.getInvoice = async () => ({
      vccJobId: 'vcc-test',
      paymentUrl: 'solana:pay?destination=G...',
  ...
  function makeAmbiguousError(solanaStatus, hash) {
  ...
  function seedStuckOrdering({ minutesAgo = 10, ctx_solana_txid = null } = {}) {
    const id = uuidv4();
    ...
    db.prepare(
      `INSERT INTO orders (
        ...
        vcc_job_id, sol_sent_at, vcc_notified_at,
        fulfillment_attempt, ctx_solana_txid,
        created_at, updated_at
      ) VALUES (?, 'ordering', '10.00', 'usdc_solana', NULL,
        'vcc-job', NULL, NULL, 0, ?, ?, ?)`,
    ).run(id, ctx_solana_txid, ts, ts);
    return id;
  }

  it('SELECT skips rows that already have ctx_solana_txid set (parked)', async () => {
    // Seed a parked row — prior attempt captured hash 'abc' and is
    // waiting on ops verification. The reconciler must NOT pick it up
    // even though sol_sent_at is still NULL.
    const id = seedStuckOrdering({ ctx_solana_txid: 'parked-hash-abc' });
    ...
    const row = db.prepare(`SELECT status, ctx_solana_txid FROM orders WHERE id = ?`).get(id);
    assert.equal(row.ctx_solana_txid, 'parked-hash-abc', 'hash must be preserved');
    assert.equal(row.status, 'ordering', 'status untouched by the skipped reconciler');
  });

  it('parks the order on retry-side solanaStatus=unknown (tx may have landed)', async () => {
    ...
    const row = db
      .prepare(`SELECT status, error, ctx_solana_txid, sol_sent_at FROM orders WHERE id = ?`)
      .get(id);
    assert.equal(row.status, 'failed');
    assert.equal(row.ctx_solana_txid, AMBIGUOUS_HASH);
    // Critical: sol_sent_at must stay NULL so a schema-only audit still
    // shows "we don't know if this paid" — but ctx_solana_txid IS set,
    // so the reconciler's SELECT filter skips future retries.
    assert.equal(row.sol_sent_at, null);
  ...
  it('parks on retry-side solanaStatus=applied_failed (tx landed but failed on chain)', async () => {
    ...
    const row = db.prepare(`SELECT status, ctx_solana_txid FROM orders WHERE id = ?`).get(id);
    assert.equal(row.status, 'failed');
    assert.equal(row.ctx_solana_txid, FAILED_HASH);
  });

  it('non-ambiguous error (no solanaStatus marker) leaves row in ordering for attempt-counter retry', async () => {
    ...
    assert.equal(row.ctx_solana_txid, null);
  });

  it('successful retry persists the returned hash to ctx_solana_txid', async () => {
    ...
    const row = db.prepare(`SELECT sol_sent_at, ctx_solana_txid FROM orders WHERE id = ?`).get(id);
    assert.ok(row.sol_sent_at, 'sol_sent_at should be set on success');
    assert.equal(row.ctx_solana_txid, 'HAPPY_RETRY_HASH');
  });
});
