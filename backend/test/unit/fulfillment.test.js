// Tests for obolus fulfillment.js — refunds, webhooks, and freeze state.
// Card delivery itself happens in the VCC service; this file covers what
// obolus still owns: scheduleRefund, fireWebhook, enqueueWebhook, isFrozen.
...
patchCache('payments/sol-sender', {
...
function seedOrder(overrides = {}) {
  ...
  db.prepare(
    `
    INSERT INTO orders (id, status, amount_usdc, payment_asset, api_key_id, sender_address, webhook_url, payment_sol_amount)
    VALUES (@id, @status, @amount_usdc, @payment_asset, @api_key_id, @sender_address, @webhook_url, @payment_sol_amount)
  `,
  ).run({
    id,
    status: 'failed',
...
describe('scheduleRefund', () => {
  ...
  it('sends USDC refund and marks order refunded', async () => {
    ...
    assert.equal(order.refund_solana_txid, 'refund_usdc_txhash');
  });

  it('sends SOL refund for sol_solana orders', async () => {
    const id = seedOrder({
      sender_address: 'GREFUND_DEST',
      payment_asset: 'sol_solana',
      payment_sol_amount: '50.00',
    });
    await scheduleRefund(id);

    const order = getOrder(id);
    assert.equal(order.status, 'refunded');
    assert.equal(order.refund_solana_txid, 'refund_xlm_txhash');
  });
  ...
  it('marks refund_pending when no sender_address', async () => {
    ...
    assert.equal(order.refund_solana_txid, null);
  });

  it('marks refund_pending when sendUsdc throws', async () => {
    xlmSenderMock.sendUsdc = async () => {
      throw new Error('Solana unavailable');
    };
    const id = seedOrder({ sender_address: 'GREFUND_DEST', payment_asset: 'usdc_solana'' });
    await scheduleRefund(id);
  ...
  it('marks refund_pending for SOL order with no payment_sol_amount', async () => {
    const id = seedOrder({
      sender_address: 'GREFUND_DEST',
      payment_asset: 'sol_solana',
      payment_sol_amount: null,
...
//
// sol-sender's submitWithRetry now annotates thrown errors with
// solanaStatus ('unknown' | 'applied_failed' | 'not_landed') and a
...
describe('scheduleRefund — F1 solanaStatus + txHash capture', () => {
  beforeEach(() => {
    resetDb();
    xlmSenderMock.sendUsdc = async () => 'refund_usdc_txhash';
    xlmSenderMock.sendXlm = async () => 'refund_xlm_txhash';
  ...
  function makeAnnotatedError(message, solanaStatus, txHash) {
    const err = /** @type {any} */ (new Error(message));
    err.solanaStatus = solanaStatus;
    err.txHash = txHash;
    return err;
  }
  ...
  it('USDC refund: unknown network outcome writes txHash but leaves refund_pending', async () => {
    ...
    // hash so an operator can verify via Solana RPC directly.
    const AMBIGUOUS_HASH = 'a'.repeat(64);
    xlmSenderMock.sendUsdc = async () => {
      throw makeAnnotatedError(
        'submit network error and Solana RPC lookup also failed',
        'unknown',
        AMBIGUOUS_HASH,
      );
    };
    ...
    assert.equal(
      order.refund_solana_txid,
      AMBIGUOUS_HASH,
      'hash must be captured so ops can verify on-chain',
    );
  });

  it('SOL refund: applied_failed (tx landed but failed) writes txHash + stays refund_pending', async () => {
    ...
    const order = getOrder(id);
    assert.equal(order.status, 'refund_pending');
    assert.equal(order.refund_solana_txid, FAILED_HASH);
  });
  ...
  it('USDC refund: not_landed (safe retry) still persists hash for audit trail', async () => {
    ...
    assert.equal(order.refund_solana_txid, NOT_LANDED_HASH);
  });

  it('legacy error without solanaStatus leaves refund_solana_txid null', async () => {
    // The pre-audit error shape — just an Error with a message, no
    // solanaStatus or txHash. Behaviour must be backwards-compatible:
    // refund_pending with null txid (no forensic data available).
    xlmSenderMock.sendUsdc = async () => {
      throw new Error('Solana unavailable'); // no markers
    };
  ...
  it('preserves a prior refund_solana_txid if already set (does not overwrite)', async () => {
    ...
    db.prepare(`UPDATE orders SET status = 'failed', refund_solana_txid = ? WHERE id = ?`).run(
      FIRST_HASH,
      id,
    );
    ...
    assert.equal(order.refund_solana_txid, FIRST_HASH);
  });
});
...
describe('fireWebhook', () => {
  ...
  it('adds HMAC signature headers when webhookSecret is provided', async () => {
    ...
    assert.ok(
      opts.headers['X-Obolus-Signature']?.startsWith('sha256='),
      'signature header present',
    );
    assert.ok(opts.headers['X-Obolus-Timestamp'], 'timestamp header present');

    // Verify the HMAC is correct
    const ts = opts.headers['X-Obolus-Timestamp'];
    const body = opts.body;
    const expected = crypto.createHmac('sha256', 'mysecret').update(`${ts}.${body}`).digest('hex');
    assert.equal(opts.headers['X-Obolus-Signature'], `sha256=${expected}`);
  });

  it('omits signature headers when no webhookSecret', async () => {
    await fireWebhook('https://hooks.example.com/events', { status: 'ok' }, null);
    const { opts } = fetchCalls[0];
    assert.ok(!opts.headers['X-Obolus-Signature']);
    assert.ok(!opts.headers['X-Obolus-Timestamp']);
  });
...
// $0.50 on a failed order. This is a financial correctness bug: obolus
// takes customer money when a fulfillment fails. The fix sums amount_usdc
// and excess_usdc in BigInt lamports and refunds the total.
...
describe('F2-fulfillment: computeUsdcRefundAmount helper', () => {
  ...
  it('adds excess to quoted amount in lamport precision', () => {
    // $10.00 + $0.50 = $10.50. In lamport precision: 100000000 + 5000000 = 105000000.
    assert.equal(_computeUsdcRefundAmount('10.00', '0.5000000'), '10.5000000');
  });

  it('handles microcent-level excess without float rounding', () => {
    // The whole reason this uses BigInt lamports: floats would round
...
describe('F2-fulfillment: refund sends amount_usdc + excess_usdc end-to-end', () => {
  ...
  it('refunds quoted amount + excess when overpaid', async () => {
    ...
    const order = getOrder(id);
    assert.equal(order.status, 'refunded');
    assert.equal(order.refund_solana_txid, 'refund_txhash');
  });
