...
//
// In test/dev mode (no OBOLUS_SECRET_BOX_KEY) seal is a pass-through and
...
describe('card-vault — F1 sealing roundtrip', () => {
  ...
  it('seals to opaque ciphertext when a secret-box key is set, and openCard round-trips it back', () => {
    ...
    const previousKey = process.env.OBOLUS_SECRET_BOX_KEY;
    process.env.OBOLUS_SECRET_BOX_KEY = crypto.randomBytes(32).toString('hex');
    delete require.cache[require.resolve('../../src/lib/secret-box')];
    delete require.cache[require.resolve('../../src/lib/card-vault')];
    try {
      const { sealCard, openCard } = require('../../src/lib/card-vault');
      ...
      if (previousKey === undefined) delete process.env.OBOLUS_SECRET_BOX_KEY;
      else process.env.OBOLUS_SECRET_BOX_KEY = previousKey;
      delete require.cache[require.resolve('../../src/lib/secret-box')];
      delete require.cache[require.resolve('../../src/lib/card-vault')];
    }
  });
