...
// auditor would try:
//   - double-@ email "a@b@obolus.xyz"
//   - substring injection "a@obolus.xyzevil.com"
//   - subdomain "a@bar.obolus.xyz"
//   - trailing dot "a@obolus.xyz."
//   - case variations
...
describe('requireInternal — domain allow', () => {
  ...
  it('allows any @obolus.xyz email', () => {
    const mw = freshMiddleware();
    const { nextCalled } = runMiddleware(mw, { id: 'u1', email: 'ops@obolus.xyz' });
    assert.equal(nextCalled, true);
  });

  it('matches the @obolus.xyz domain case-insensitively', () => {
    const mw = freshMiddleware();
    const { nextCalled } = runMiddleware(mw, { id: 'u1', email: 'OPS@OBOLUS.COM' });
    assert.equal(nextCalled, true);
  });

  it('rejects non-@obolus.xyz emails when INTERNAL_EMAILS is unset', () => {
    const mw = freshMiddleware();
...
describe('requireInternal — domain suffix bypass guards', () => {
  ...
  it('rejects substring injection: a@obolus.xyzevil.com', () => {
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@obolus.xyzevil.com' });
    assert.equal(statusCode, 403);
  });

  it('rejects subdomain: a@bar.obolus.xyz', () => {
    // endsWith('@obolus.xyz') on 'a@bar.obolus.xyz' — last 14
    // chars are 'r.obolus.xyz', doesn't match @obolus.xyz.
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@bar.obolus.xyz' });
    assert.equal(statusCode, 403);
  });

  it('rejects trailing dot: a@obolus.xyz.', () => {
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@obolus.xyz.' });
    assert.equal(statusCode, 403);
  });

  it('rejects lookalike tld: a@obolus.co', () => {
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@obolus.co' });
    assert.equal(statusCode, 403);
  });

  it('rejects prefix injection: xobolus.xyz as suffix', () => {
    const mw = freshMiddleware();
    const { statusCode } = runMiddleware(mw, { id: 'u1', email: 'a@xobolus.xyz' });
    assert.equal(statusCode, 403);
  });
});
