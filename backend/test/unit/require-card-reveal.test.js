...
//   - OBOLUS_CARD_REVEAL_EMAILS unset → 403 (fail-closed default)
//   - OBOLUS_CARD_REVEAL_EMAILS set but user not in list → 403
...
describe('requireCardReveal — auth gate', () => {
  ...
  beforeEach(() => {
    origEnv = process.env.OBOLUS_CARD_REVEAL_EMAILS;
  });

  afterEach(() => {
    if (origEnv === undefined) delete process.env.OBOLUS_CARD_REVEAL_EMAILS;
    else process.env.OBOLUS_CARD_REVEAL_EMAILS = origEnv;
  });

  it('401 when req.user is missing', () => {
    delete process.env.OBOLUS_CARD_REVEAL_EMAILS;
  ...
  it('401 when req.user has no email (F2 defence)', () => {
    ...
    process.env.OBOLUS_CARD_REVEAL_EMAILS = 'ops@obolus.xyz';
    const mw = freshMiddleware();
...
describe('requireCardReveal — allowlist', () => {
  ...
  it('403 card_reveal_disabled when env var is empty string', () => {
    process.env.OBOLUS_CARD_REVEAL_EMAILS = '';
    const mw = freshMiddleware();
    const { statusCode, body } = runMiddleware(mw, { id: 'u1', email: 'ops@obolus.xyz' });
    assert.equal(statusCode, 403);
    assert.equal(body.error, 'card_reveal_disabled');
  });
  ...
  it('calls next() when user email is in the allowlist', () => {
    process.env.OBOLUS_CARD_REVEAL_EMAILS = 'ops@obolus.xyz,other@obolus.xyz';
    const mw = freshMiddleware();
  ...
  it('matches email case-insensitively', () => {
    process.env.OBOLUS_CARD_REVEAL_EMAILS = 'Ops@Obolus.Com';
    const mw = freshMiddleware();
    const { nextCalled } = runMiddleware(mw, { id: 'u1', email: 'ops@obolus.xyz' });
    assert.equal(nextCalled, true);
  });
  ...
  it('trims whitespace around env entries', () => {
    process.env.OBOLUS_CARD_REVEAL_EMAILS = '  ops@obolus.xyz  ,  other@obolus.xyz  ';
    const mw = freshMiddleware();
    const { nextCalled } = runMiddleware(mw, { id: 'u1', email: 'other@obolus.xyz' });
    assert.equal(nextCalled, true);
  });

  it('re-reads env on every call (supports live rotation)', () => {
    const mw = freshMiddleware();
    process.env.OBOLUS_CARD_REVEAL_EMAILS = 'first@obolus.xyz';
    assert.equal(runMiddleware(mw, { id: 'u1', email: 'first@obolus.xyz' }).nextCalled, true);
    // Rotate without re-requiring the module.
    process.env.OBOLUS_CARD_REVEAL_EMAILS = 'second@obolus.xyz';
    assert.equal(
      runMiddleware(mw, { id: 'u1', email: 'first@obolus.xyz' }).nextCalled,
      false,
      'first@ should be rejected after rotation',
    );
    assert.equal(
      runMiddleware(mw, { id: 'u1', email: 'second@obolus.xyz' }).nextCalled,
      true,
...
describe('F1-card-reveal: bizEvent observability', () => {
  ...
  it('emits card_reveal.denied with reason=env_not_configured when allowlist is empty', () => {
    ...
    const mw = freshMiddleware();
    runWithReq(mw, {
      user: { id: 'u1', email: 'ops@obolus.xyz' },
      ip: '10.0.0.3',
      headers: {},
    });
    const denied = events.find((e) => e.name === 'card_reveal.denied');
    ...
    assert.equal(denied.fields.actor_email, 'ops@obolus.xyz');
  ...
  it('emits card_reveal.allowed on successful authorization', () => {
    ...
    assert.equal(allowed.fields.actor_email, 'ops@obolus.xyz');
...
describe('F3-card-reveal: bizEvent failure does not alter auth verdict', () => {
  ...
  it('still calls next() on allowed branch even though bizEvent throws', () => {
    process.env.OBOLUS_CARD_REVEAL_EMAILS = 'ops@obolus.xyz';
    const mw = freshMiddleware();
    const req = { user: { id: 'u1', email: 'ops@obolus.xyz' }, ip: '10.0.0.1', headers: {} };
    let statusCode = null;
    const res = {
      status(c) {
        statusCode = c;
