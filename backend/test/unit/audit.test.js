...
describe('audit log', () => {
  ...
  // so a BigInt in details (Solana i128 amounts, SUM() over large columns
  ...
  it('serialises BigInt details values to strings (not _serialise_failed)', () => {
    recordAudit({
      ...
      details: {
        solanaAmount: 12345678901234567890n,
        nested: { count: 42n },
        normal: 'string',
      },
    });
    ...
    assert.equal(parsed.solanaAmount, '12345678901234567890');
