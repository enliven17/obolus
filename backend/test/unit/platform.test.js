...
describe('isPlatformOwner', () => {
  ...
  beforeEach(() => {
    original = process.env.OBOLUS_PLATFORM_OWNER_EMAIL;
  });

  afterEach(() => {
    if (original === undefined) {
      delete process.env.OBOLUS_PLATFORM_OWNER_EMAIL;
    } else {
      process.env.OBOLUS_PLATFORM_OWNER_EMAIL = original;
    }
  });
  ...
  it('returns false for a non-matching email', () => {
    process.env.OBOLUS_PLATFORM_OWNER_EMAIL = 'ops@obolus.test';
    assert.equal(isPlatformOwner('user@example.com'), false);
  });

  it('returns true for an exact email match', () => {
    process.env.OBOLUS_PLATFORM_OWNER_EMAIL = 'ops@obolus.test';
    assert.equal(isPlatformOwner('ops@obolus.test'), true);
  });
  ...
  it('F1: returns false on an object input', () => {
    process.env.OBOLUS_PLATFORM_OWNER_EMAIL = 'ops@obolus.test';
    assert.equal(isPlatformOwner(/** @type {any} */ ({ email: 'ops@obolus.test' })), false);
  });

  it('F1: returns false on an array input', () => {
    process.env.OBOLUS_PLATFORM_OWNER_EMAIL = 'ops@obolus.test';
    assert.equal(isPlatformOwner(/** @type {any} */ (['ops@obolus.test'])), false);
  });

  it('F1: returns false when OBOLUS_PLATFORM_OWNER_EMAIL is set to a non-string', () => {
    ...
    /** @type {any} */ (process.env).OBOLUS_PLATFORM_OWNER_EMAIL = 42;
    assert.equal(isPlatformOwner('ops@obolus.test'), false);
  });
  ...
  it('F2: returns false when configured env is whitespace-only', () => {
    process.env.OBOLUS_PLATFORM_OWNER_EMAIL = '   ';
    assert.equal(isPlatformOwner('   '), false);
    assert.equal(isPlatformOwner('ops@obolus.test'), false);
    assert.equal(isPlatformOwner(''), false);
  });
