...
describe('auto-registration', () => {
  ...
  it('registers with VCC when no token is cached, then fetches the invoice', async () => {
    ...
    global.fetch = async (url, opts) => {
      ...
      return makeResponse(201, {
        job_id: 'job-1',
        payment_url: 'web+solana:pay?destination=G...&amount=5',
      });
    };
    ...
    assert.match(paymentUrl, /^web\+solana:pay/);
  ...
  it('skips registration and uses cached token on second call', async () => {
    ...
    fetchResponse = makeResponse(201, { job_id: 'job-2', payment_url: 'web+solana:pay?x=1' });
    global.fetch = async (url, opts) => {
      fetchCalls.push({ url, opts });
      return fetchResponse;
    };
...
describe('getInvoice', () => {
  beforeEach(() => {
    ...
    fetchCalls.length = 0;
    fetchResponse = makeResponse(201, { job_id: 'job-inv', payment_url: 'web+solana:pay?x=1' });
  ...
  it('returns { vccJobId, paymentUrl } mapped from vcc response', async () => {
    fetchResponse = makeResponse(201, {
      job_id: 'job-42',
      payment_url: 'web+solana:pay?mapped=1',
    });
    const result = await getInvoice('order-map', '5.00');
    assert.equal(result.vccJobId, 'job-42');
    assert.equal(result.paymentUrl, 'web+solana:pay?mapped=1');
  });
