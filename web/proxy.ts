...
//      cookie themselves.
//
//   2. status.obolus.xyz subdomain — everything under this host
...
const STATUS_HOST = 'status.obolus.xyz';

// Match /obolus.svg, /icon.png, /skill.md, /robots.txt, /sitemap.xml,
// /humans.txt, /manifest.webmanifest, /.well-known/*, etc. Anything
// with a dot in the last path segment is a static file that should
// pass through unchanged; rewriting /obolus.svg to /status would return
...
export function proxy(request: NextRequest) {
  ...
  if (host === STATUS_HOST || host.startsWith(`${STATUS_HOST}:`)) {
    ...
    //   - API routes (/api/*)
    //   - Well-known endpoints (/.well-known/ etc.)
...
export const config = {
  // Match everything under status.obolus.xyz except the Next.js
  // asset pipeline. The main apex host (obolus.xyz) hits the
  ...
  matcher: [
    {
      ...
      has: [{ type: 'header', key: 'host', value: 'status.obolus.xyz' }],
    },
  ],
};
