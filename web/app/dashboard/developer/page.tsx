    ...
    <PageContainer>
      ...
      <Card title="Send a test webhook">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
          ...
          <div style={{ fontSize: '0.7rem', color: 'var(--fg-dim)', lineHeight: 1.5 }}>
            ...
            secret, we'll sign it with <code>X-Obolus-Signature: sha256=hex(HMAC(ts.body))</code>{' '}
            and <code>X-Obolus-Timestamp</code>.
          </div>
          <Button
            variant="primary"
            onClick={runTest}
      ...
      <Card title="Integration snippets">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
          ...
          >
            The <code>obolus</code> npm package ships a CLI for onboarding and a TypeScript SDK
            for purchases. See{' '}
            <a
              href="/docs"
              target="_blank"
          ...
          <Snippet title="Install" code={`npm install obolus@latest`} />
          <Snippet
            title="Onboard an agent"
            code={`npx -y obolus@latest onboard --claim <claim-code>`}
          />
          <Snippet
            title="Purchase a card"
            code={`import { purchaseCardOWS } from 'obolus';
...
function verify(req, secret) {
  const ts = req.headers['x-obolus-timestamp'];
  const sig = req.headers['x-obolus-signature']?.replace('sha256=', '');
