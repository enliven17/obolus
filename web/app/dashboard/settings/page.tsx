...
const NOTIF_PREF_KEY = 'obolus.notifications';
const DENSITY_KEY = 'obolus.density';
...
export default function SettingsPage() {
  ...
  function exportData() {
    ...
    const data = {
      ...
      agentGroups: window.localStorage.getItem('obolus.agent_groups'),
    };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `obolus-settings-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  function clearLocalData() {
    ...
    window.localStorage.removeItem('obolus.agent_groups');
    window.localStorage.removeItem(NOTIF_PREF_KEY);
    window.localStorage.removeItem(DENSITY_KEY);
    window.localStorage.removeItem('obolus.theme');
...
// Platform-owner-only treasury card. Renders the obolus Solana treasury
// public key + live Solana RPC balance + a copy button + a solana.expert link.
...
function PlatformTreasuryCard() {
  ...
  const [balance, setBalance] = useState<{ sol: string; usdc: string } | null>(null);
      ...
      .then(async (w) => {
        ...
        try {
          ...
          if (!res.ok) {
            ...
            if (!cancelled) setBalance({ sol: '0', usdc: '0' });
            return;
          }
          const data = (await res.json()) as {
            balances: { asset_type: string; asset_code?: string; balance: string }[];
          };
          const sol = data.balances.find((b) => b.asset_type === 'native')?.balance ?? '0';
          const usdc =
            data.balances.find(
              (b) => b.asset_type === 'credit_alphanum4' && b.asset_code === 'USDC',
            )?.balance ?? '0';
          if (!cancelled) setBalance({ sol, usdc });
        } catch (err) {
  ...
  return (
    <Card title="Platform treasury" padding="1.25rem 1.5rem">
      ...
      >
        The Obolus treasury wallet on Solana. Top this up with SOL or USDC when fulfillment runs
            ...
            <>
              <Row label="SOL balance">
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.78rem' }}>
                  {parseFloat(balance.sol).toFixed(2)}
                </span>
              </Row>
              ...
              onClick={() => {
                const explorer =
                  wallet.network === 'testnet'
                    ? `https://solana.expert/explorer/testnet/account/${wallet.public_key}`
                    : `https://solana.expert/explorer/public/account/${wallet.public_key}`;
                window.open(explorer, '_blank');
              }}
            >
              View on solana.expert ↗
            </Button>
          </div>
        </div>
      )}
