    ...
    if (query.trim()) {
      ...
      list = list.filter((o) => {
        return (
          ...
          (o.solana_txid || '').toLowerCase().includes(q)
        );
      });
    }
    return list;
  ...
  return (
    <Drawer open={true} onClose={onClose} title={`Order ${order.id.slice(0, 8)}`} width={460}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {order.solana_txid && (
          <Row label="Solana txid">
            <a
              href={`https://solana.expert/explorer/public/tx/${order.solana_txid}`}
              target="_blank"
              rel="noreferrer"
              style={{
                fontFamily: 'var(--font-mono)',
            ...
            >
              {order.solana_txid.slice(0, 10)}…{order.solana_txid.slice(-8)} ↗
            </a>
          </Row>
        )}
