...
export function CreateAgentDrawer({ open, onClose }: Props) {
  ...
  const step = useMemo<Step>(() => {
    ...
    const hasFunds = balance ? parseFloat(balance.sol) >= 1 || parseFloat(balance.usdc) > 0 : false;
  ...
  const snippet = created?.claim
    ? [
        'Read /skill.md and set up this agent by running:',
        `npx -y obolus@latest onboard --claim ${created.claim.code}`,
      ].join('\n')
    : '';
      ...
      {!created ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          >
            ...
            />
            <StepRow state={stepState(step, 'wallet')} title="Solana wallet created" />
            <StepRow
              state={stepState(step, 'awaiting_deposit')}
              title="Awaiting deposit"
              detail="Send SOL to activate the wallet and cover rent. To receive USDC, the agent must first open a token account (use devnet SOL faucet: solana airdrop 2)."
            />
            <StepRow
              ...
              detail={
                balance
                  ? `${parseFloat(balance.sol).toFixed(4)} SOL · ${parseFloat(balance.usdc).toFixed(2)} USDC`
                  : undefined
              }
            />
          ...
          {walletAddress && (
            ...
            >
              ...
              >
                Send SOL to the address above (use: solana airdrop 2 --url devnet).
                receive USDC, the agent must first open a USDC token account by running{' '}
                <code>obolus wallet token account</code>. The stepper will flip to{' '}
                <strong>Funded</strong> automatically once Solana RPC sees the deposit.
              </div>
            </div>
          )}
