// Platform treasury — live Solana RPC balances + recent outflows.
      ...
      {data && (
        <>
          <KpiRow>
            <KpiTile
              label="SOL"
              value={data.balance.sol !== null ? parseFloat(data.balance.sol).toFixed(4) : '—'}
            />
            <KpiTile
              label="USDC"
              value={data.balance.usdc !== null ? parseFloat(data.balance.usdc).toFixed(4) : '—'}
            />
            <KpiTile label="Network" value={process.env.NEXT_PUBLIC_SOLANA_NETWORK || 'devnet'} />
          ...
          <Card title="Public key">
            ...
            {data.balance.public_key && (
              <div style={{ marginTop: '0.6rem', display: 'flex', gap: '0.5rem' }}>
                <a
                  href={`https://solana.expert/explorer/public/account/${data.balance.public_key}`}
                  target="_blank"
                  rel="noopener"
            ...
            {data.balance.error && (
              <div style={{ color: 'var(--red)', fontSize: '0.72rem', marginTop: '0.4rem' }}>
                Solana RPC error: {data.balance.error}
              </div>
            )}
          </Card>
          ...
          <Card title={`Recent outflows (${data.outflows.length})`} padding={0}>
            {data.outflows.length === 0 ? (
              <EmptyState
                ...
                description="Solana RPC returned no outgoing payments for this account."
              />
            ) : (
              <table>
                <thead>
                  ...
                  {data.outflows.map((o) => (
                    <tr key={o.tx_hash + o.amount}>
                      ...
                      <td style={{ fontSize: '0.66rem' }}>
                        <a
                          href={`https://solana.expert/explorer/public/tx/${o.tx_hash}`}
                          target="_blank"
                          rel="noopener"
