...
//   3. Solana RPC balances polled on a 30s interval for funded wallets
...
async function fetchSolana RPCBalance(publicKey: string, network?: string): Promise<WalletBalance> {
  try {
    ...
    // 5s timeout so a hung Solana RPC lookup doesn't stall the entire
    ...
    for (const b of data.balances || []) {
      if (b.asset_type === 'native') sol = b.balance;
      if (
...
export function DashboardProvider({ children }: { children: ReactNode }) {
  ...
  // Solana RPC balances for every agent that has a wallet address. Polls
