...
export default function AgentDetailPage({ params }: PageProps) {
  ...
  const balSol = walletBalances[liveAgent.id]?.sol || '0';
  const balUsdc = walletBalances[liveAgent.id]?.usdc || '0';
  const comboUsd = parseFloat(balUsdc) + parseFloat(balSol) * 0.2; /* rough SOL/USD heuristic */
      ...
      >
        ...
        <KpiRow>
          ...
          <KpiTile
            ...
            hint={`${parseFloat(balSol).toFixed(2)} SOL · ${parseFloat(balUsdc).toFixed(2)} USDC`}
          />
        </KpiRow>
      ...
      >
        <section>
          ...
          <div style={{ fontSize: '0.7rem', color: 'var(--fg-dim)', marginTop: 2 }}>
            {parseFloat(balSol).toFixed(4)} SOL · {parseFloat(balUsdc).toFixed(2)} USDC
          </div>
        ...
        {agent.wallet_public_key ? (
          ...
          >
            <div style={{ fontSize: '0.78rem', color: 'var(--fg-dim)', textAlign: 'center' }}>
              Send SOL or USDC to this address to fund the agent. Funds show up on the dashboard
              within ~30 seconds.
            </div>
