// Server-only helpers for the obolus admin dashboard session.
//
...
export const ADMIN_SESSION_COOKIE = 'obolus_admin_session';
...
/**
 * Resolve the upstream obolus backend URL from env. Prefers
 * `OBOLUS_BACKEND_URL` (server-only) and falls back to
...
export function getBackendBaseUrl(): string {
  const url = process.env.OBOLUS_BACKEND_URL ?? process.env.NEXT_PUBLIC_API_BASE_URL;
  if (!url) {
    if (process.env.NODE_ENV === 'production') {
      throw new Error('OBOLUS_BACKEND_URL (or NEXT_PUBLIC_API_BASE_URL) is required');
    }
